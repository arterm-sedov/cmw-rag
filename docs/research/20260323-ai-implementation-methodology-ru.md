...
**Рекомендуемый ответ:** опереться на целевую операционную модель (роли, KPI, риски), поэтапный POC → Pilot → Scale, пакет отчуждения (код, конфигурация, данные, модели, runbook, обучение) и блок комплаенса (152-ФЗ, NIST AI RMF, guardrails), а также на **единую промышленную наблюдаемость** (трассировки и метрики по этапам RAG и агента, учёт токенов) с политикой данных, согласованной с ПДн. Глобальные шлюзы для coding agents (**OpenRouter**, **OpenCode Zen**) относить к **разработке и экспериментам**, а не к подразумеваемому продакшн-API для ПД в РФ без отдельной оценки — см. «Ориентиры для заказчика» и Compliance. Детали — в разделах TOM, внедрение, **Промышленная наблюдаемость**, отчуждение и Compliance ниже.
...
* **LLMOps / AI Architect:** Проектирование инфраструктуры (vLLM/MOSEC), мониторинг качества (RAGAS/DeepEval), целевая архитектура **телеметрии** (трассировки, метрики токенов и латентности, политика сэмплинга и ретенции) и согласование с ИБ при контурах с ПДн.
