...
**Рекомендуемый ответ:** опереться на целевую операционную модель (роли, KPI, риски), поэтапный POC → Pilot → Scale, пакет отчуждения (код, конфигурация, данные, модели, runbook, обучение) и блок комплаенса (152-ФЗ, NIST AI RMF, guardrails). Глобальные шлюзы для coding agents (**OpenRouter**, **OpenCode Zen**) относить к **разработке и экспериментам**, а не к подразумеваемому продакшн-API для ПД в РФ без отдельной оценки — см. «Ориентиры для заказчика» и Compliance. Детали — в разделах TOM, внедрение, отчуждение и Compliance ниже.
...
**Контекст для внедрения у заказчика в РФ:** значение «по умолчанию» для OpenRouter отражает **удобство upstream-репозитория и разработки** (единый API, быстрые эксперименты), а не рекомендацию **промышленного** контура при персональных данных и требованиях суверенитета. В таких случаях целевой выбор инференса — **API российских облаков**, **on-prem** или иной маршрут из блока Compliance ниже и из парного [отчёта по сайзингу](20260323-sizing-capex-opex-ru.md).
...
Раздел основан на действующих сведениях о законодательстве по состоянию на **23.03.2026** и учитывает актуальные
...
**Статус:** на **23.03.2026** речь идёт о **проекте** федерального закона, а не о действующей норме. Первичная фиксация проекта — на портале НПА: [проект № 166424](https://regulation.gov.ru/projects#npa=166424) (актуальная редакция, сроки обсуждения и текст — только по материалам портала).
...
**Трансграничные шлюзы разработки** (OpenRouter, OpenCode Zen и аналоги) для **продакшн**-решений с ПД в РФ **не подменяют** маркировку, локализацию и логику реестра — см. «Ориентиры для заказчика: инструменты ускорения разработки» и парный [отчёт по сайзингу](20260323-sizing-capex-opex-ru.md).
