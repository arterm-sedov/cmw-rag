...
**Статус:** обновление от 2026-03-23
...
**Рекомендуемый ответ:** начинать с облачного POC по российским тарифам, затем при стабильной утилизации **>40–60%** рассчитывать break-even on-prem; опираться на дерево факторов стоимости, сценарный сайзинг и раздел TCO ниже, включая **строки наблюдаемости** (телеметрия, ретенция, сэмплинг) как часть OpEx. Глобальные дайджесты (a16z, зарубежные прайсы) использовать только как **рыночный фон**, не как baseline КП. Зарубежные API-шлюзы разработки (**OpenRouter**, **OpenCode Zen**) не смешивать с тарифами облаков РФ в одной модели **продакшн**-TCO — см. раздел **«Глобальные шлюзы разработки»** и блок **«Зарубежные API»** в OpEx.
...
Для управляемости бюджета ИИ-нагрузок рекомендуется закрепить **unit economics** в духе практик [FinOps Foundation](https://www.finops.org/wg/generative-ai/): стоимость **на 1 млн токенов**, **на пользователя/месяц**, **на успешный диалог** или **на тикет** — в разрезе среды (облако РФ vs on-prem). **Аллокация** затрат по продуктам/департаментам и **наблюдаемость** (токены, латентность, ошибки, объём и срок хранения трасс/аудита агентских шагов и вызовов инструментов) позволяют связывать рост контекста и вызовов инструментов с линией P&L. При наличии инструментирования по [OpenTelemetry GenAI metrics](https://opentelemetry.io/docs/specs/semconv/gen-ai/gen-ai-metrics/) гистограмма **`gen_ai.client.token.usage`** с атрибутами типа токена (`input` / `output`) и модели даёт воспроизводимую основу для сверки с **billable** токенами у провайдера API; **`gen_ai.server.time_to_first_token`** и **`gen_ai.server.time_per_output_token`** помогают отделять узкое место **инференса** от стоимости **ретрива** в отчётах по юнит-экономике. Ниже в разделе «Юнит-экономика» эти метрики развиваются количественно; оптимизации (маршрутизация моделей, кэширование промптов, батчинг) следует оценивать через тот же учёт.
