...
Упоминания **coding agents** (в т.ч. Cursor, OpenCode, OpenWork, **OpenRouter**) в этом разделе отражают **рыночный контекст и OpEx разработки** на стороне заказчика; они **не** являются baseline **продакшн-RAG** и **не** входят в SKU референс-стека CMW — см. парное резюме [Методология внедрения и отчуждения ИИ](20260323-ai-implementation-methodology-ru.md), подраздел «Ориентиры для заказчика: инструменты ускорения разработки».
