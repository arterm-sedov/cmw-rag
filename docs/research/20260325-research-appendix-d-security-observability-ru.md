...
- **Интернет для coding-agent?** полный выход в сеть или только **доверенные** зеркала (Git, пакеты) и артефакты;
