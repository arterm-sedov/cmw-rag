...
# Приложение D. Безопасность, комплаенс и observability {: #app_d__root }
## Обзор комплекта {: #app_d__pack_overview }

Приложение задаёт **периметр контроля** для промышленного GenAI: модель угроз, комплаенс (в т.ч. ПДн, 152-ФЗ), промышленная **observability** и практические паттерны для RAG и агентов. Основные отчёты дают **управленческие выводы**; здесь — детализация для **решений по ИБ и эксплуатации**. **Валюта в сметах** — [приложение A](./20260325-research-appendix-a-index-ru.md#app_a_fx_policy).
...
Ориентиры **токенов/с** из публикаций сообщества и обзоров полезны лишь для грубой прикидки до замеров; в эксплуатации их следует сопоставлять с **`gen_ai.server.time_per_output_token`** (и смежными метриками сервера) на целевом стенде. Иллюстративные таблицы по локальному железу и MoE — в **«Основной отчёт: сайзинг и экономика»**.
