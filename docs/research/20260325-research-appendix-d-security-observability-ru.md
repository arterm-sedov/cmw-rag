...
## Обзор комплекта {: #app_d__pack_overview }

Приложение задаёт **периметр контроля** для промышленного GenAI: модель угроз, комплаенс (в т.ч. ПДн, 152‑ФЗ), промышленная **наблюдаемость (observability)** и практические паттерны для RAG и агентов. Основные отчёты дают **управленческие выводы**; здесь — детализация для **решений по ИБ и эксплуатации**. **Валюта в сметах** — [приложение A](./20260325-research-appendix-a-index-ru.md#app_a_fx_policy).
...
Ориентиры **токенов/с** из публикаций сообщества и обзоров полезны лишь для грубой прикидки до замеров; в эксплуатации их следует сопоставлять с метриками времени генерации токенов на целевом стенде. Иллюстративные таблицы по локальному железу и MoE — в «**Основной отчёт: сайзинг и экономика**».
...
- **Предпрогретые среды или контейнеры с более слабой изоляцией?** компромисс **стоимости и надёжности** warm pool против экономии на границе доверия — закладывать в TCO и модель рисков (см. _«[Основной отчёт: сайзинг и экономика](./20260325-research-report-sizing-economics-main-ru.md#sizing_cost_factor_tree)»_);
