...
## Обзор комплекта {: #research_pkg_d_obzor_paketa }
...
Ориентиры **токенов/с** из публикаций сообщества и обзоров полезны лишь для грубой прикидки до замеров; в эксплуатации их следует сопоставлять с **`gen_ai.server.time_per_output_token`** (и смежными метриками сервера) на целевом стенде. Иллюстративные таблицы по локальному железу и MoE — в **«Основной отчёт: сайзинг и экономика»** (комплект v1).
...
Ниже — паттерны, влияющие на стоимость контекста, выбор CLI vs толстых протоколов, память агента и наблюдаемость в продакшене; при поставке **корпоративный RAG-контур** / **агентный слой платформы (CMW Platform)** они сверяются с целевой операционной моделью и комплектом отчуждения, а не принимаются как обязательный «золотой стек».
...
### Справочно: граница доверия, сеть и среда исполнения агента {: #research_pkg_d_spravochno_granitsa_doveriya_set_i_sreda_ispolneniya_agenta }
...
**Принцип управляемой плоскости исполнения:** наружу из среды исполнения операционно предпочтительно выводить **артефакты, журналы аудита и результаты проверок**, а не необузданные прямые действия по prod-системам; любой прямой доступ агента к боевым контурам — через явные политики, брокеры доступа и наблюдаемые шлюзы. См. также _«[Основной отчёт: методология внедрения и разработки](./20260325-research-report-methodology-main-ru.md#research_methodology_20260325_inzheneriya_obvyazki_dlya_agentov)»_ (инженерия обвязки) и раздел **«Промышленная наблюдаемость LLM, RAG и агентов»** выше по настоящему документу.

### Справочно: модель риска, паттерны среды и минимальный состав платформы {: #research_pkg_d_spravochno_model_riska_patterny_sredy_i_minimalnyi_sostav_platformy }
...
### Справочно: управляемые песочницы, сравнение моделей и бенчмарки {: #research_pkg_d_spravochno_upravlyaemye_pesochnitsy_sravnenie_modelei_i_benchmarki }
...
### Справочно: безопасный MVP контура исполнения за 30 дней, дискуссия по средам и выводы {: #research_pkg_d_spravochno_bezopasnyi_mvp_kontura_ispolneniya_diskussiya_sredy_vyvody }
...
- **Прямые изменения или только артефакты?** запись в репозиторий **напрямую** vs передача **diff** и отчётов на **ревью** (см. _«[Приложение B: отчуждение ИС и кода (KT, IP)](./20260325-research-appendix-b-ip-code-alienation-ru.md#research_pkg_b_spravochno_agent_v_pr_i_artifacty_vmesto_pryamoi_zapisi_v_is)»_);
- **Предпрогретые среды или контейнеры с более слабой изоляцией?** компромисс **стоимости и надёжности** warm pool против экономии на границе доверия — закладывать в TCO и модель рисков (см. _«[Основной отчёт: сайзинг и экономика](./20260325-research-report-sizing-economics-main-ru.md#research_sizing_20260325_derevo_faktorov_stoimosti_cost_factor_tree)»_);
...
- Логистика: +10-20%
- Российские премиум-наценки на H100, A100, RTX 4090 (в т.ч. кастом **48 ГБ**), профессиональные SKU (**RTX PRO 6000** и аналоги)
...
### Зрелость GenAI в маркетинге РФ (CMO Club × red_mad_robot, 2025) {: #research_pkg_d_zrelost_genai_v_marketinge_rf_cmo_club_red_mad_robot_2025 }
...
### Управляемые песочницы, gVisor и исследования container runtime {: #research_pkg_d_upravlyaemye_pesochnitsy_gvisor_i_issledovaniya_container_runtime }
