...
В данном документе представлен подробный анализ аппаратных требований (сайзинга), капитальных затрат (CapEx) и операционных затрат (OpEx) для развертывания экосистемы ИИ Comindware с фокусом на **российский рынок**, **локальные облака** и **суверенный** контур по умолчанию. Анализ опирается на **сведения из референс-репозиториев Comindware** по **сервер инференса MOSEC** и **инференс на базе vLLM**, на **публичные тарифы** российских облачных платформ (Cloud.ru, Yandex Cloud, SberCloud, Selectel Foundation Models Catalog, MWS GPT) и на **публичные внешние бенчмарки и обзоры**; при планировании ёмкости учитывайте нагрузку со стороны **корпоративный RAG-контур** (RAG) и **агентный слой платформы (Comindware Platform)** (агентные сценарии платформы). Итоговые ёмкости для КП требуют **замеров на стенде заказчика**.
...
Упоминания **coding agents** (в т.ч. Cursor, OpenCode, OpenWork, **OpenRouter**) в этом разделе отражают **рыночный контекст и OpEx разработки** на стороне заказчика; они **не** являются baseline **продакшн-RAG** и **не** входят в SKU референс-стека Comindware — см. сопутствующее резюме **Методология внедрения и отчуждения ИИ**, подраздел «Ориентиры для заказчика: инструменты ускорения разработки».
...
**Связь с TCO и КП:** цифры **Memex** и **KARL** выше отражают **бенчмарки в публикациях**, а не гарантию для коммерческого предложения. Строки CapEx/OpEx для **корпоративный RAG-контур** и **агентный слой платформы (Comindware Platform)** пересчитывают по **профилю заказчика**, трассам и согласованным eval, иначе смешиваются сценарии с разной глубиной агента и разным качеством.
...
### Инструменты разработки с ИИ (ориентиры, вне SKU Comindware) {: #sizing_ai_dev_tools_benchmarks }
