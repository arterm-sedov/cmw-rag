...
В данном документе представлен подробный анализ аппаратных требований (сайзинга), капитальных затрат (CapEx) и операционных затрат (OpEx) для развертывания экосистемы ИИ CMW с фокусом на российский рынок и локальные облачные провайдеры. Анализ основан на фактических измерениях VRAM из **cmw-mosec**, спецификациях vLLM из **cmw-vllm** и тарифах российских облачных платформ (Cloud.ru, Yandex Cloud, SberCloud, Selectel Foundation Models Catalog, MWS GPT); при планировании ёмкости учитывайте нагрузку со стороны **cmw-rag** (RAG) и **cmw-platform-agent** (агентные сценарии платформы).
...
*   **Агенты:** для встроенных инструментов AI Studio токены инструментов тарифицируются отдельно (см. тот же документ); при смете для **cmw-platform-agent** закладывать запас по всем типам токенов из прайса.
