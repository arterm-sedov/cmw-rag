...
\*См. [профиль on-prem GPU в проектах CMW](#sizing_onprem_gpu_profile_cmw): на **кастомной 4090 48 ГБ** ориентир «свободно» выше примерно на **+24 ГБ** к значению колонки; **PRO 6000 96 ГБ** — пересчёт по замерам.
...
-   **Оборудование:** RTX 4090 Desktop Workstation (реф. **24 ГБ** VRAM; в контурах CMW также **48 ГБ** кастом — см. [профиль on-prem GPU](#sizing_onprem_gpu_profile_cmw))
...
-   **Оборудование:** RTX 4090 Workstation (24 ГБ реф.; 48 ГБ кастом — см. [профиль on-prem GPU](#sizing_onprem_gpu_profile_cmw))
...
4. **Локальный инференс:**
   ...
   - **RTX 4090:** референсный GeForce **24 ГБ** достаточен для многих конфигураций **0.6B–4B**; в проектах CMW — также **48 ГБ** кастом и старший класс **PRO 6000 Blackwell 96 ГБ** (см. [профиль on-prem GPU](#sizing_onprem_gpu_profile_cmw))
   - Для тяжёлых 8B+ и длинного контекста без агрессивной квантизации часто нужны A100/H100 или **96 ГБ**-класс — по замерам

### Бенчмарки RTX 4090 (реф. 24 ГБ GeForce; у CMW — 48 ГБ кастом) {: #sizing_rtx_4090_benchmarks }
...
### Coding Agents {: #sizing_coding_agents }
...
### AI Infrastructure {: #sizing_ai_infrastructure }
...
### Robotics & Hardware {: #sizing_robotics_hardware }
...
### Enterprise AI {: #sizing_enterprise_ai }
...
### TCO Calculator: On-Prem vs Cloud {: #sizing_tco_calculator_on_prem_cloud }
...
### GPU Hardware Pricing {: #sizing_gpu_hardware_pricing }
...
### Depreciation и Hardware Lifecycle {: #sizing_depreciation_hardware_lifecycle }
...
## VRAM Requirements для LLM Inference {: #sizing_vram_requirements_llm_inference }
...
### Throughput Estimation {: #sizing_throughput_estimation }
...
## Практические рекомендации по сайзингу (Decision Tree) {: #sizing_sizing_recommendations_decision_tree }

### Decision Tree для выбора инфраструктуры {: #sizing_infrastructure_selection_decision_tree }
...
### Порог утилизации: On-Prem vs Cloud {: #sizing_utilization_threshold_onprem_cloud }
