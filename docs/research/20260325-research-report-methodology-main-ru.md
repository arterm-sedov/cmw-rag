...
- [«Основной отчёт: сайзинг и экономика»](./20260325-research-report-sizing-economics-main-ru.md#sizing_pack_overview) (детализированные цифры и TCO)
...
**Основания и границы применимости:** **инженерная практика Comindware** — открытые репозитории **cmw-rag**, **cmw-mosec**, **cmw-vllm**, **cmw-platform-agent** (код, конфигурации, README). **Рыночные цифры, опросы и бенчмарки** — из публичных и отраслевых материалов; они **не** отражают замеры Comindware у конкретных заказчиков, если такие проекты отдельно не задокументированы. **Синтез и рекомендации** — экспертиза Comindware по внедрению и сопровождению под резидентный контур РФ (152-ФЗ, локализация, локальные облака) с учётом ограничений зарубежных источников для вашего контура.

### Валюта {: #method_fx_policy }

Пересчёт USD — [приложение A](./20260325-research-appendix-a-index-ru.md#app_a_fx_policy). Тарифы, TCO и таблицы — [основной отчёт: сайзинг и экономика](./20260325-research-report-sizing-economics-main-ru.md#sizing_pack_overview).

---

## Резюме для руководства {: #method_exec_summary }
...
- **Глобальный фон внедрения (экосистема OpenAI, не норма РФ):** публичная эмпирика «_[The state of enterprise AI](https://openai.com/index/the-state-of-enterprise-ai-2025-report)_» (телеметрия enterprise-клиентов и опрос) показывает темп и разрыв «передовые vs медиана»; детали — в «_[Эмпирика корпоративного внедрения](./20260325-research-report-methodology-main-ru.md#method_openai_implementation_report)_». **Ограничения выборки:** цифры отражают **выборку экосистемы OpenAI**, **не** типовой контур резидентного продакшна в РФ и **не** заменяют КП без учёта ПДн, тарифов РФ и политики заказчика.
...
Материалы канала **@Redmadnews** (лаборатория **red_mad_robot**, начало 2026) задают **управленческий фон** для обсуждения масштабирования и организации R&D; они **не** подменяют договорную модель поставки из _«[Приложение B: отчуждение ИС и кода](./20260325-research-appendix-b-ip-code-alienation-ru.md#app_b_pack_overview)_»_ и **не** являются верифицированными KPI для коммерческого предложения.
...
- **AI-first организация:** подкаст с руководством иллюстрирует постановку продуктовой и корпоративной модели «вокруг ИИ»; из него **не** следует состав поставки **корпоративный RAG-контур**, **агентный слой платформы (Comindware Platform)** или обязательный стек инференса. См. «_[AI-first стратегия: подкаст (пост канала)](https://t.me/Redmadnews/5170)_»; полный выпуск — «_[Подкаст «Ноосфера» #129 (YouTube)](https://www.youtube.com/watch?v=jTKhg1jqF_M)_».
...
Управленческие тезисы по стратегии внедрения, барьерам и программам обучения руководителей (в т.ч. ориентир **СКОЛКОВО**) изложены в разделе _«[Стратегия внедрения ИИ и организационная зрелость](./20260325-research-report-methodology-main-ru.md#method_ai_strategy_org_maturity)»_ выше; **этот** раздел посвящён инженерным и R&D-сигналам лаборатории **red_mad_robot**.

### Подход к ИИ-коду в бизнесе {: #method_ai_code_business_approach }
...
### Оптимизация рассуждений моделей {: #method_model_reasoning_optimization }
...
### Память и контекст в ИИ-агентах {: #method_ai_agent_memory_context }
...
- Два агента из одной базовой модели: **curriculum** генерирует задачи, **executor** учится решать их с инструментами; ко-эволюция без опоры на фиксированный внешний датасет — ориентир **НИОКР** и будущих контуров дообучения, **не** поставляемый компонент **корпоративный RAG-контур** / **агентный слой платформы (Comindware Platform)**.
...
### Инфраструктура навыков ИИ {: #method_ai_skills_infrastructure }
...
### R&D-практики {: #method_rd_practices }
...
Отдельно в открытом контуре той же лаборатории описан **MCP Tool Registry**: центральный реестр MCP-серверов как архитектурный каркас для сборки и управления RAG- и агентными сценариями; код выложен на GitHub. Для методологии передачи заказчику это согласуется с уже зафиксированными строками комплекта отчуждения по **конфигурации MCP, allowlist и CI** — реестр можно использовать как **внешний ориентир** при проектировании каталога допустимых серверов, без подмены им корпоративного контура и политики ИБ.

### Исследования марта 2026 (справочно) {: #method_march_2026_research_reference }
...
### Инструменты для агентов (neuraldeep) {: #method_agent_tools_neuraldeep }
...
### Обвязка разработки (llm_under_hood) {: #method_development_wrapper_llm_under_hood }
...
### Практики разработки с AI (virrius_tech_chat) {: #method_ai_development_practices_virrius }
...
### Новости индустрии (Март 2026) {: #method_industry_news_march_2026 }

**Модели:**

- **Cursor Composer 2** — **платный ориентир рынка, не продукт Comindware**; код на уровне Opus 4.6 / GPT-5.4; в публичных прайсах фигурирует порядок **~42,5 ₽/млн** входных токенов (эквивалент **~0,5 USD/млн** при пересчёте [**85 ₽/USD**](#method_fx_policy)). Контекст вспомогательных инструментов заказчика — в подразделе «Ориентиры для заказчика: инструменты ускорения разработки» выше.
...
### Практики AI Coding (How2AI, Тимур Хахалев) {: #method_ai_coding_practices_how2ai }
...
### AI-автоматизация для бизнеса (ROИИ) {: #method_ai_business_automation_roii }
...
**Онтология сегментов (управленческая сетка):** при чтении отраслевых обзоров удобно разделять **инфраструктуру и вычисления** (облако GPU, on-prem кластеры), **базовые и прикладные модели**, **продукты и сервисы** поверх моделей, **интеграцию и внедрение** — чтобы не смешивать CapEx на железо, OpEx на API и проектный бюджет на изменение процессов.
...
**Сценарный контур до 2030 (не числовой прогноз комплекта):** в обзорных материалах встречают три **логические** ветки — (1) **консервативный** рост при ограниченном доступе к ускорителям и усилении регулирования; (2) **консолидация** вокруг ограниченного числа экосистемных игроков; (3) **ускоренная индустриализация** при снятии узких мест по железу и кадрам. Для заказчика это **шаблон стресс-теста** портфеля (POC → Pilot → Scale), а не baseline для КП; количественные ряды — только из указанных первоисточников в _«[Основной отчёт: сайзинг — ИИ-рынок России](./20260325-research-report-sizing-economics-main-ru.md#sizing_russia_ai_market_stats_forecasts)_»_.
...
По формулировкам первоисточника, внедрение ускоряется в **ширину и глубину**: за год еженедельные сообщения в ChatGPT Enterprise выросли примерно **в 8 раз**, средний работник отправляет **на 30%** больше сообщений; использование структурированных сценариев (Projects, Custom GPTs) выросло **в 19 раз** с начала года — как сдвиг от разовых запросов к повторяемым процессам. Среднее потребление **reasoning tokens** на организацию за **12** месяцев выросло примерно **в 320 раз** — сигнал для учёта **дорогих** режимов инференса в FinOps.

Отдельно фиксируется **разрыв** между «передовыми» и медианой: у работников около **95-го** перцентиля **в 6 раз** больше сообщений, чем у медианного сотрудника; у «передовых» организаций **в 2 раза** больше сообщений на место и более глубокая кросс-командная интеграция. Для заказчиков в РФ это **глобальный** ориентир неравенства внедрения, а не норматив для КП.
...
**Оговорки:** материал отражает **экосистему OpenAI** и **не** подменяет требования 152-ФЗ, суверенитет данных и выбор контура в РФ. Отраслевые и географические акценты в отчёте полезны для приоритизации гипотез, но **не** как baseline отечественного рынка без отдельной верификации.
...
*Оценки в руб. из USD при **85 ₽/USD** ([валюта комплекта](./20260325-research-appendix-a-index-ru.md#app_a_fx_policy)); фактический TCO — по контракту и курсу на дату закупки.*
...
### AGORA: Industrial AI и Enterprise {: #method_agora_industrial_ai_enterprise }
...
### AI & грабли: Agile-подход к ИИ-внедрению {: #method_agile_ai_implementation }
...
### How2AI: Российские ИИ-инструменты {: #method_russian_ai_tools }
...
### Публичные материалы @Redmadnews (ориентиры управления, не КП) {: #method_redmadnews_management_guidelines }
