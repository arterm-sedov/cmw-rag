...
Этот файл (`tasks/`) — **инструкция для автора**, а `report-pack/` — **читательский продукт для C-level**.

- **Главная цель пакета:** дать руководству Comindware материалы, которые помогают принимать решения и формировать офферы при продаже/передаче экспертизы Comindware по ИИ (внедрение, отчуждение, экономика, комплаенс, риски).
- **Кому пишем в `report-pack/`:** CEO/COO/CFO/CTO, коммерческие и технические руководители, которые принимают решение по бюджету, архитектуре, модели поставки и ответственности.
- **Какой результат считается правильным:** текст можно взять в бриф, презентацию, коммерческое предложение или переговорную позицию **без дополнительных пояснений от автора**.
- **Что запрещено в `report-pack/`:** любые авторские/редакторские ремарки и «мета»-формулировки (например, «в этом документе мы описали…», «для автора», «смотри раздел задачи», «эта версия для редактирования», ссылки на правила написания, на workflow и на устройство репозитория как на часть narrative).
- **Границы внутренних ссылок:** внутренние пути, `.md`-ссылки и репозиторные указатели допустимы только в глубоком комплекте, где это нужно для навигации авторов; в кратких C-level резюме и внешне передаваемых версиях — не использовать (кроме редких, явно обоснованных исключений для проверяемости тезиса).

### 0.1 Быстрая проверка перед сохранением файла из `report-pack/`

- Можно ли дать этот текст руководителю без объяснения «как читать файл»?
- Есть ли в тексте прямой ответ на «что это значит для решения и оффера Comindware»?
- Нет ли внутри авторских ремарок, редакторских инструкций и task-oriented wording?
- Все ли ключевые тезисы имеют доказательную опору и понятные ограничения применимости?
- Сохранён ли бизнес-фокус: **enable decision and offer design**, а не «показать процесс подготовки отчёта»?
