...
graph TD
    ...
    E -->|6. ToolCall: запрос на обогащение| C
    C -->|7. Вызов внешнего API| G
    C -->|7ʹ. Вызов поискового API| H
    G -->|8. Данные контрагента| C
    H -->|8ʹ. Семантический контекст| C
    C -->|9. Результат обогащения| E
    E -->|10. Структурированный вывод| C
    C -->|11. Запись атрибутов через API| A
...
!!! question "LLM — не имеет памяти и связи с внешним миром"
    ...
    Все инструментальные вызовы идут через оркестратор (шаги 6→7→8→9).
...
1. **Триггер:** **Comindware Platform** передаёт ID записи через REST API.
2. **ACK:** агент немедленно подтверждает приём и продолжает работу в фоновом потоке. Платформа не ждёт результата.
...
5. **Анализ:** оркестратор передаёт текст документа, необходимый контекст и инструкции по его обработке в LLM. Если контекста недостаточно, LLM формирует `ToolCall`; оркестратор выполняет вызов и возвращает результат.
6. **Запись:** оркестратор записывает структурированный результат в атрибуты записи **Comindware Platform** через API.
...
Агент использует асинхронную модель fire-and-forget: **Comindware Platform** получает подтверждение немедленно, а агент выполняет обработку в фоновом потоке и записывает результат в атрибуты записи по завершении.

**Входящий запрос (триггер):**
