...
| :--- | :--- | :--- |
| **CMW Platform** | Источник документов и получатель результата | Инстанс платформы с API |
| **REST API эндпоинт** | Точка входа для автоматизации | Принимает ID записи |
| **Document Connector** | Извлекает документ из CMW | Читает вложение по ссылке |
| **Document Processor** | Конвертирует PDF/DOCX/XLSX → Markdown | Универсальный парсер |
| **LLM Agent** | Генерирует структурированное резюме | Китайская модель GLM-5 |
| **Web Search** | Конкурентная разведка: цены, погода, статистика | Автоматический вызов при необходимости |
| **Output** | Резюме → атрибут в CMW | Markdown или HTML |

### Поток данных

```mermaid
graph LR
    A[CMW<br/>Platform] -->|record ID| B[REST API<br/>endpoint]
    B --> C[Document<br/>Connector]
    C --> D[Document<br/>Processor]
    D -->|text extraction| E[LLM Agent<br/>+ Web Search]
    E -->|summary| F[CMW<br/>→ result field]
...
| **LLM** | Z-AI GLM-5 через OpenRouter API |
...
| Параметр | Описание |
| :--- | :--- |
| **Application** | Приложение CMW (настраивается в YAML) |
...
4. **Шаг 4:** Показать заполненный атрибут резюме в интерфейсе CMW.
...
- **Несколько CMW-инстансов:** один агент, несколько платформ — через YAML
