...
!!! question "Определения"
    ...
    - **Токен** — минимальная единица текста, которую обрабатывает LLM. Токен может быть целым словом, частью слова, отдельным символом или знаком препинания. Токенизатор (алгоритм разбиения текста на токены) преобразует текст в числовые ID — **модель оперирует только числами**, никогда не видит фактический текст и не знает букв и слов как таковых. 1 токен ≈ ¾ слова в английском языке.
    - **Промпт** — запрос к LLM на естественном языке с инструкциями и данными для обработки.
    - **Агент** — в данном контексте: автономный сервис обработки документов. В Lukoil/Support-агенте это FastAPI-приложение (`rag_engine/api/app.py`) с эндпоинтами `/api/v1/cmw/...`. Принимает ID записи, координирует обработку через LLM и инструменты (через `create_summary_agent()` из `agent_factory.py`), записывает результат в **Comindware Platform**. Понятия «агент» и «оркестратор» в нашем коде — синонимы: агент = оркестратор = LangChain-объект, управляющий вызовами инструментов.
    - **Операции** — действия, которые агент выполняет в **Comindware Platform**: чтение/запись атрибутов, создание записей, вычисления.
...
LLM — предиктивный генератор токенов (см. [Терминология](#terminology)). Она **не** читает файлы, **не** выполняет вычисления и **не** обращается к сети. Агентные возможности обеспечивает детерминированная программная оснастка.

Модель формулирует запрос к внешнему источнику; агент выполняет вызов необходимых инструментов и возвращает результат модели для дальнейшего анализа:

| Компонент | Решение | Операции |
| :--- | :--- | :--- |
| **LLM** | Какой инструмент вызвать, с какими параметрами | Только текстовая генерация |
| **Агент** (оркестратор) | Какой шаг следующий, обработка ошибок, повторы | Все инструментальные вызовы, API-запросы, вычисления |

!!! tip "Ключевой принцип: LLM не имеет памяти и связи с внешним миром"

    Модель лишь предсказывает токены, у неё нет никакого внутреннего представления о текущем бизнес-контексте. Более того, модель **не** знает даже текущее время или год — ей необходимы внешние контекстные данные для решения любых бизнес-задач. Модель принимает решения; оркестратор их исполняет. Контекст и память сеанса поддерживает оркестратор, а **не LLM**. Подробнее — см. [Терминология](#terminology).
