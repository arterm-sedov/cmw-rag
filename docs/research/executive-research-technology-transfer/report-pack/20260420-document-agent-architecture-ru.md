...
Агент оперирует записями **Comindware Platform** напрямую через API, минуя экранные формы и может вычислять и заполнять любые значения с помощью всех возможностей Python (включая написание и исполнение одноразового кода на ходу в песочнице):
...
Агенту необходимы отдельный аккаунт и отдельная отдельная роль для выполнения и атрибуции действий в **Comindware Platform**, непрерывного аудита транзакций агента и сквозной трассировки API-вызовов.
