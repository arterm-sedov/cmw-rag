...
!!! question "Определения"
    ...
    - **Токен** — минимальная единица текста, которую обрабатывает LLM. Токен может быть целым словом, частью слова, отдельным символом или знаком препинания. Токенизатор (программа разбиения текста) преобразует текст в числовые ID — модель работает только с числами, никогда не видит фактический текст и не знает букв и слов как таковых. 1 токен ≈ ¾ слова в английском. Большинство API тарифицируют по токенам.
...
!!! tip "Ключевой принцип: LLM не имеет памяти и связи с внешним миром"
    ...
    Ключевые определения — см. выше. Модель **не** знает даже текущее время или год — ей необходимы внешние контекстные данные для решения любых бизнес-задач.

    Модель принимает решения; оркестратор их исполняет. Контекст и память сеанса поддерживает оркестратор, а **не LLM**.
