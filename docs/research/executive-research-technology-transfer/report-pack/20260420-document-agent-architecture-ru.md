...
5. **Анализ:** агент передаёт текст документа, необходимый контекст и инструкции по его обработке в LLM. Если контекста недостаточно, LLM формирует один или несколько вызовов инструментов (`ToolCall` на диаграмме, это **запрос модели к агенту на выполнение действий в реальном мире**) для обогащения и аналитики данных; агент выполняет вызовы и возвращает результаты в цикле или параллельно.
