...
Схемы взаимодействия (YAML) задаются декларативно на стороне агента, а не в путях передачи данных **Comindware Platform**:
...
Агент оперирует записями **Comindware Platform** напрямую через API, минуя экранные формы. Декларативная YAML-конфигурация определяет атрибуты, но агент агент ограничен ролевой моделью платформы.
