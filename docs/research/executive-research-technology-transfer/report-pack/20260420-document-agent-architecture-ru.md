...
!!! question "Определения"
    ...
    - **Агент** — программный слой, обеспечивающий взаимодействие пользователя, **Comindware Platform** и LLM. В нашем коде: `create_summary_agent()` из `agent_factory.py` создаёт LangChain-агент, координирующий вызовы LLM и инструментов.
    - **Оркестратор** — компонент, который управляет конвейером обработки и вызовов LLM, обрабатывает ошибки и повторы. В нашем коде: классы `PlatformConnector` и `DocumentSummaryConnector` оркестрируют полный цикл: получить запись → извлечь документ → обработать через агента → записать результат.

## Архитектура агента {: #doc_agent_agent_architecture }

### Компоненты агента {: #doc_agent_architecture_components }

См. [Определения](#terminology). Агент и оркестратор — это **разные** компоненты в нашей архитектуре (см. определения выше). Они работают вместе: оркестратор координирует высокоуровневый цикл (fetch → process → write), агент управляет вызовами LLM и инструментов внутри обработки.
...
См. [Определения](#terminology) для понимания ключевых терминов.
