...
!!! note "Как работает языковая модель"
    ...
    **Для реализации агентной логики необходим детерминированный оркестратор (каркас):**

    - **Файлы** → модель воспринимает исключительно извлеченный текст; прямая обработка бинарных форматов (PDF/DOCX) технически невозможна.
    - **Поиск** → требует интеграции с внешними API (Yandex, Tavily, Exa). Модель формулирует запрос, а оркестратор маршрутизирует его и возвращает ответ.
    - **Вычисления** → LLM подвержены галлюцинациям при строгих расчетах. Детерминированные операции делегируются профильным библиотекам Python (например, `datetime`, `math`).

    Таким образом, Агент представляет собой симбиоз LLM и инструментального оркестратора. Без последнего модель остается исключительно генератором текста, лишенным интерактивности.
...
Дублирование конфигурационных схем на стороне платформы (маршрутизация) и агента (валидация) признано избыточным. Принятая архитектура делегирует агенту проактивное чтение данных:

- Платформа транслирует исключительно ID записи, минимизируя конфигурационные накладные расходы.
- Агент автономно извлекает целевой документ, контекстные инструкции и метаданные.
- Агент фиксирует результат в платформе посредством прямого API-вызова.

**Асинхронный паттерн:** платформа инициирует триггер и продолжает выполнение бизнес-процесса. Агент асинхронно агрегирует данные, проводит аналитику и осуществляет обратную запись результата.

При проектировании критически важно **демаркировать зоны ответственности**:

- **Comindware Platform** выполняет детерминированные бизнес-процессы (строгая валидация, маршрутизация, вычисления).
- Агент активируется на участках, требующих эвристического анализа, семантической обработки неструктурированных документов и динамической маршрутизации запросов к внешним системам.
...
- Системные инструкции (промпты) для LLM.

Управление бизнес-логикой осуществляется декларативно, исключая необходимость модификации исходного кода.
...
- Атрибут с контекстной инструкцией (обязательно — задаёт конкретную бизнес-задачу для LLM).
