...
Документ обобщает **методологию внедрения и отчуждения** решений класса корпоративных ИИ-ассистентов с RAG, локальным или облачным инференсом и агентными сценариями.

**Термины:** названия **корпоративный RAG-контур**, **сервер инференса на базе MOSEC/vLLM**, агентный слой **Comindware Platform** используются как **условные обозначения ролей компонентов** иллюстративного референс-стека, а не как коммерческое обоснование готового продукта. Полные подписи строк экосистемы (в т.ч. «Сервер инференса на базе …») — _«[Приложение C](./20260325-research-appendix-c-cmw-existing-work-ru.md#app_c_pack_overview)»_.
...
**Граница применения:** используйте документ для выбора сценария внедрения, модели управления рисками и условий передачи контура; **утвердите** финальное инвестиционное обязательство на этапе коммерческого согласования по актуальным параметрам сделки.
...
- Опирайтесь на фазы внедрения (PoC → Пилот → Масштабирование) и целевую операционную модель (TOM) как рамку для бюджета и этапов.
- Используйте критерии приёмки и KPI для измеримого контроля качества.
- Используйте матрицу аргументов по ролям ЛПР у заказчика (CEO/CFO/CIO/CISO/CRO/CPO), чтобы адаптировать коммерческие и технологические аргументы.

**Для переговоров с C-Level:**

- Показывайте трёхэтапную модель (Проектирование → Эксплуатация → Отчуждение) как гарантию передачи способности.
- Для CIO/CTO: опирайтесь на архитектурные паттерны RAG, инференса и агентов как на техническую основу КП.
...
- **Проблема:** без явного **периметра до LLM** (минимизация и обезличивание входа, разделение вспомогательных и основной модели, политика телеметрии) растут риски по 152-ФЗ и стоимость инцидентов; без **офлайн и онлайн eval** невозможно доказуемо связывать смену модели или индекса с качеством и бюджетом.
- **Вопрос для решения:** как внедрять и масштабировать ассистентов на стеке **корпоративный RAG‑контур**, **сервер инференса на базе MOSEC/vLLM** и агентный слой **Comindware Platform**, и **как отчуждать** экспертизу и артефакты клиенту без потери управляемости.
- **Рекомендуемый ответ:**
    - опереться на целевую операционную модель (роли, KPI, риски), поэтапный PoC → Пилот → Масштабирование, комплект отчуждения (код, конфигурация, данные, модели, runbook, обучение) и блок комплаенса (152-ФЗ, приказ Роскомнадзора № 140, NIST AI RMF, guardrails), а также на единую промышленную наблюдаемость (трассировки, метрики, учёт токенов);
...
При планировании ИИ-проектов учитывайте:
...
| **Сентябрь 2027** | Проект ФЗ об ИИ — **ст. 21:** вступление в силу **1 сентября 2027** (текст проекта на НПА; окончательная дата — в принятом акте) | **ст. 12** — маркировка синтезированного контента; **ст. 12 ч. 4** — площадки **>100 тыс.** суточной аудитории в РФ; **ст. 10 ч. 3 п. «г»** — сервисы **>500 тыс.** обращений/сутки → обязанности по **ст. 10.1** ФЗ № 149-ФЗ; **ст. 7–8** — суверенные/национальные модели и реестр доверенных |

Подробнее — см. _Приложение D «[Безопасность, комплаенс и observability](./20260325-research-appendix-d-security-observability-ru.md#app_d__pack_overview)»_.
...
Политика применения: принимайте решения по модели внедрения и передаче контура по этим опорам; отклонения допустимы только при явной фиксации причины и даты пересмотра.
...
Связь с юнит-экономикой и TCO при многошаговых агентских цепочках — см. в отчёте _«[Сайзинг и экономика](./20260325-research-report-sizing-economics-main-ru.md#sizing_finops_unit_economics)»_.

Для **пилотов линии поддержки** полезно опереться на **примерные расчёты расхода токенов**, полученные из доступных данных по публичному корпусу заявок с портала поддержки: таблицы, допущения по переводу слов в токены и разводку **целевой цены за тикет** и **верхней модельной оценки цикла** приведены в отчёте _«[Сайзинг и экономика](./20260325-research-report-sizing-economics-main-ru.md#sizing_token_consumption_estimates)»_, подраздел «**Примерные расчёты расхода токенов на доступных данных (портал поддержки)**»; здесь цифры не дублируются.
...
Типичный **разрыв зрелости:** интерес к ИИ и сами инструменты уже есть, а **системные процессы внедрения, обучения и поддержки команд** ещё не выстроены — это стыкуется с фазами **PoC → Пилот → Масштабирование** ниже, с комплектом отчуждения и обучением (см. _«[Приложение B: отчуждение ИС и кода](./20260325-research-appendix-b-ip-code-alienation-ru.md#app_b_pack_overview)»_).
...
- **LLMOps / AI Architect:** Проектирование инфраструктуры (vLLM/MOSEC), мониторинг качества (RAGAS/DeepEval), целевая архитектура **телеметрии** (трассировки, метрики токенов и латентности, политика сэмплинга и ретенции) и согласование с ИБ при контурах с ПДн; совместно с владельцами разработки — **среда для агентов** (инструменты, линтеры, CI, контуры офлайн/eval и при необходимости мультиагентных циклов разработки — см. _«[Инженерия обвязки для агентов](./20260325-research-report-methodology-main-ru.md#method_agent_wrapper_engineering)»_).
...
- **Аргумент для продаж и закупки:** участие **MTS AI** и других игроков показывает, что отрасль движется к стандартизации eval; это усиливает обоснование выбора моделей для заказчика в цикле PoC -> пилот -> масштабирование.
- **Как использовать в проекте:** дополняйте внешними ориентирами (MERA) внутренний контур оценки заказчика (RAGAS, DeepEval, LLM-as-judge), а не заменяйте его при фиксации KPI качества в проектной документации.
- **Требование к отчуждению и воспроизводимости:** разбор цикла улучшения **Cotype** с опорой на LLM-судей (_«[Хабр, MTS AI](https://habr.com/ru/companies/mts_ai/articles/892176/)»_) используйте как методологический референс; включайте в пакет передачи промпты моделей-судей, эталоны и регрессионные наборы.

### Публичные рыночные сигналы и коммерческие выводы {: #method_market_benchmarks_2026 }

Материалы канала лаборатории **red_mad_robot** **[@Redmadnews](https://t.me/Redmadnews)** **используйте** как внешний индикатор рыночного спроса для решений C-level и приоритизации сценариев внедрения/отчуждения ИИ.
...
- **Сигнал 3 — постоянный поток инженерных новинок и R&D-дайджестов:**
  технологический горизонт меняется быстрее типового цикла закупки.
    ...
    **Вывод для Comindware:** используйте такие материалы как методический радар, но не как замену продуктовым требованиям, архитектурным ограничениям и составу поставки по КП.
...
**Используйте** 4-фазный подход, основанный на практиках **red_mad_robot** и **Just AI**:
...
- **Инструментарий:** оптимизированный инференс, внедрение Guardrails, согласование нагрузки со стороны корпоративного RAG-контура и агентного слоя **Comindware Platform**.
...
| **Сервер инференса (специализированный)** | **На базе MOSEC** | Обслуживание специализированных моделей: эмбеддинга, реранкера и охранника | MOSEC, PyTorch |
| **Сервер инференса (универсальный)** | **На базе vLLM** | Обслуживание LLM | vLLM, CUDA |
...
- **MOSEC** — открытый фреймворк **подачи ML-моделей через HTTP API**: быстрый веб-слой (Rust), логика воркеров на Python, динамическое батчирование запросов, поэтапные пайплайны и облачно-ориентированные практики (прогрев, graceful shutdown, метрики).
...
Этот раздел предназначен для руководителей, которые принимают решение о том, как упаковать предложение **Comindware** по внедрению и отчуждению ИИ-экспертизы в контуре заказчика: приоритет — РФ-резидентность данных, управляемый риск комплаенса и предсказуемая экономика владения. При жёстких требованиях к размещению данных и инфраструктуры типовой контур — **российские управляемые платформы** (перечень ниже) и/или **закрытый контур** (on-prem, выделенный сегмент у провайдера) в рамках политики заказчика и применимых норм.
...
- **Коммерческая цель:** используйте единый набор проверяемых аргументов в пресейле и в переговорах о передаче (KT/IP), не дублируя и не рассинхронизируя тарифные цифры между документами.
- **Принцип чтения блока:** здесь фиксируются роли провайдеров, составы модельных линеек и правила сверки SKU; все расчётные значения и сценарный сайзинг вынесены в сопутствующий документ.

!!! tip "Как использовать раздел в диалоге с C-level"

    Для расчётов и ценовых сравнений используйте единый канонический источник: _«[Краткое изложение: Оценка сайзинга, CapEx и OpEx для клиентов (российский рынок)](./20260325-research-report-sizing-economics-main-ru.md#sizing_russian_ai_cloud_tariffs)»_. В нём собраны все количественные тарифы (руб. за токены, комплекты, руб./час GPU), дерево факторов стоимости и сценарный сайзинг.

    Ориентиры по **покупке и почасовой аренде GPU** (в т. ч. поставщики вне токенных прайсов FM) — в _«[Цены на GPU-оборудование (покупка и аренда)](./20260325-research-report-sizing-economics-main-ru.md#sizing_gpu_hardware_pricing_all)»_; контекст **топологии ёмкости и классов источников цифр** — _«[Топология ёмкости GPU и типы источников цифр](./20260325-research-report-sizing-economics-main-ru.md#sizing_gpu_capacity_topology_bench_classes)»_.
...
- **Каталог (на [странице продукта](https://cloud.ru/products/evolution-foundation-models) перечислены позиции с идентификаторами Hugging Face `org/repo`):**
  ...
  - **GigaChat:** коммерческие SKU `GigaChat`, `GigaChat Lite`, `GigaChat Pro`, `GigaChat-2-Max` и ветка `ai-sage/GigaChat3-10B-A1.8B` (для SKU-to-Hub мэппинга проверять соответствие по официальному каталогу и карточке модели на дату сверки).
...
- **Тарифы:** первоисточник — [правила тарификации AI Studio](https://aistudio.yandex.ru/docs/ru/ai-studio/pricing.html): таблица Model Gallery, ₽ **с НДС** за **1000** токенов (входящие, кеш, инструменты, исходящие); для агентов — отдельно токены инструментов. Эквиваленты **₽/млн** и строки по моделям — в сопутствующем резюме. Публикации СМИ (например, ориентиры порядка ~0,5 ₽ за 1000 токенов) — для справки; в КП опирайтесь только на официальный прайс на дату сверки.
...
| VK Cloud ML | Нет каталога LLM в документации | BYO на ML Platform | Инфраструктура под **сервер инференса на базе MOSEC/vLLM** |
...
| **GigaChat (открытые веса Сбера, `ai-sage`)** | [GigaChat3-10B-A1.8B](https://huggingface.co/ai-sage/GigaChat3-10B-A1.8B) (3.0) · [GigaChat3.1-10B-A1.8B](https://huggingface.co/ai-sage/GigaChat3.1-10B-A1.8B); крупный чекпойнт: [GigaChat3.1-702B-A36B](https://huggingface.co/ai-sage/GigaChat3.1-702B-A36B) | **MIT** на публичных весах; **GigaChat API** (SberCloud) и self-host — разный TCO (см. блок **SberCloud** выше и _«[Открытые веса и API: влияние на TCO](./20260325-research-report-sizing-economics-main-ru.md#sizing_open_weights_api_tco_impact)»_) |
...
Все **числовые** ориентиры по управляемым API — в _«[Краткое изложение: Оценка сайзинга, CapEx и OpEx для клиентов (российский рынок)](./20260325-research-report-sizing-economics-main-ru.md#sizing_russian_ai_cloud_tariffs)»_ (раздел «**Тарифы российских облачных провайдеров ИИ**»). Сравнение экономики **открытых весов** и **GigaChat API** — в блоке **SberCloud** выше и в _«[Открытые веса и API: влияние на TCO](./20260325-research-report-sizing-economics-main-ru.md#sizing_open_weights_api_tco_impact)»_.
...
4. **Мониторинг и наблюдаемость:** Отслеживайте точность поиска, релевантность контекста и частоту галлюцинаций; закрепите **трассировки по этапам RAG/агента** и **метрики токенов и задержек** в духе раздела _«[Промышленная наблюдаемость LLM, RAG и агентов](./20260325-research-appendix-d-security-observability-ru.md#app_d__llm_rag_agent_observability)_» и статьи _[OpenTelemetry GenAI](https://opentelemetry.io/docs/specs/semconv/gen-ai/gen-ai-spans/)_, с политикой данных для ПДн.
...
Ниже собраны переносимые управленческие и инженерные идеи по материалам **Ozon Tech** (Хабр, анонсы митапов).

**Классический ML в поиске и рекламе, а также сценарные чат-боты с навыками, не тождественны GenAI/RAG** — используйте материалы как **аналогии** для ретривала, платформенного внедрения и MLOps, а не как замену стандартам вроде NIST AI RMF, практикам FinOps и принятому в организации комплекту отчуждения.
...
- **Qwen/Qwen3.5-35B-A3B** — рекомендуемый минимум для агентных RAG-систем [Hugging Face](https://huggingface.co/Qwen/Qwen3.5-35B-A3B). Эталонный пайплайн в репозитории [SGR Agent Core](https://github.com/vamplabAI/sgr-agent-core/tree/tool-confluence)
...
- Агентный RAG на локальных моделях (например `Qwen3.5-35B-A3B`)
...
**Проблема:** RAG работает как «плоский суп» из фактов, не понимая внутренней логики.

**Решение:** Внедрение теории риторических структур (RST). Модель понимает, где аргумент, где противоречие, где условие:
...
**Проблема:** Огромный контекст убивает TTFT (Time to First Token) и «съедает» KV-Cache.

**Решение:** Сжатие «сырых» чанков в компактные эмбеддинги через RoBERTa + селективное расширение через RL-политику.

**Результат:** Ускорение в **30 раз**. Оптимально для Tier-1 систем с миллионами запросов, где важна скорость.

### Cog-RAG: Гиперграфы и «тематическое» мышление {: #method_cog_rag_hypergraphs }

**Проблема:** Неспособность базовых систем мыслить «от общего к частному».

**Решение:** Двойные гиперграфы (темы и сущности) для имитации человеческого подхода.

**Результат:** Win Rate выше на **84.5%** по сравнению с обычным RAG. Мощно, но дорого по индексации (идеально для медицины и науки).

### HippoRAG 2: Экономим на графах в 12 раз {: #method_hipporag2_graph_efficiency }

**Проблема:** Высокие затраты на токены при индексации графов в GraphRAG системах.

**Решение:** Dual-Node архитектура (узлы-сущности + узлы-пассажи). Стек: `pip install hipporag`.

**Результат:** Снижение затрат на токены при индексации в **12 раз** (9 млн токенов vs 115 млн).
...
**Проблема:** Ручной и неэффективный промпт-инжиниринг.

**Решение:**
- **DSPy 3:** LLM как вычислительное устройство. Архитектор описывает Signatures, система генерирует и оптимизирует код промпта.
- **GEPA (Genetic-Pareto Prompt Optimizer):** Генетические алгоритмы для «скрещивания» лучших промптов и языковая рефлексия (модель анализирует свои ошибки текстом).
...
**Проблема:** Необходимость качественной разметки (Markdown / HTML / LaTeX) сложной документации и рукописей для RAG.

**Решение:** Использование профильных OCR-моделей (Nemotron-Parse, Chandra, dots.ocr).

**Результат:** Покрытие от высоконагруженных систем до архивной оцифровки:
...
**Проблема:** Дорогая монополия GPU на инференс, сложность развертывания на Edge-устройствах.

**Решение:** 1-битные LLM (1-бит веса для Attention/MLP слоев + 8/16 бит для активаций).
...
(Не «убийца GPU» для обучения, но подтачивает монополию GPU на инференс).
...
Применение агентных сценариев с **браузерной автоматизацией** и анализом снимков экрана в чувствительных средах масштабирует риски утечек, но управляется в рамках **единой политики комплаенса**. Для соответствия 152-ФЗ к артефактам автоматизации применяются базовые принципы контроля телеметрии: **строгая минимизация** собираемых ПДн, ограничение сроков хранения, изоляция контура и ролевая матрица доступа — подробнее см. _«[Персональные данные и содержимое в телеметрии (152-ФЗ)](./20260325-research-appendix-d-security-observability-ru.md#app_d__personal_data_telemetry_152fz)»_ и _«[Периметр до LLM: минимизация данных, обезличивание и обратимые подстановки](./20260325-research-appendix-d-security-observability-ru.md#app_d__llm_perimeter_data_minimization)»_ в _«[Приложение D: безопасность, комплаенс и observability](./20260325-research-appendix-d-security-observability-ru.md#app_d__llm_rag_agent_observability)»_.
...
Опирайтесь на **детальные шаблоны промптов** (иллюстрация — «_[Генерация BPMN 2.0 XML (промпт-шаблон)](https://github.com/yksi12/prompts/blob/main/generate-bpmn-prompt.md)_») и **обязательную валидацию** открытием в Camunda Modeler, [bpmn.io](https://bpmn.io/) или эквиваленте перед включением артефакта в комплект передачи.
...
**Связь с FinOps:** мультиагентные циклы и длительные прогоны разработки увеличивают **токены и wall-clock**; ориентиры по закладке в TCO — в сопутствующем резюме **Оценка сайзинга, CapEx и OpEx для клиентов**.
...
Ниже — **переносимые идеи** из открытых инженерных и отраслевых материалов, а не рекомендация конкретных поставщиков или продуктов. Используйте их как ориентиры зрелости для внедрений в духе **корпоративный RAG-контур** и агентных сценариев.
...
**Оговорки:** материал отражает экосистему OpenAI; при применении в РФ **учтите** 152-ФЗ, суверенитет данных и выбор контура. Отраслевые и географические акценты полезны для приоритизации гипотез.
...
- **0-30 дней:** Выбор 2-3 приоритетных бизнес-кейсов; PoC на связке **корпоративный RAG-контур** + **сервер инференса MOSEC**; при кейсах платформы — пилот агентный слой **Comindware Platform**; замер базового ROI.
- **30-60 дней:** Расширение пилота (**корпоративный RAG-контур**, при необходимости агентный слой **Comindware Platform**) на департамент, внедрение обсервабилити (Arize Phoenix), начало обучения команды клиента.
...
| **Методологическое** | Runbooks, процессы, тестирование, eval | 12–24 мес. |
| **Человеческое** | Обучение, сертификация, передача компетенций | Пост-Scale |
