---
title: 'Отчёт. Методология внедрения и разработки'
date: 2026-03-29
...
# Отчёт. Методология внедрения и разработки {: #method }

## Введение и резюме для руководства {: #method_intro_and_summary }
...
- Опираться на фазы внедрения (PoC → Pilot → Scale) и целевую операционную модель (TOM) как рамку для бюджета и этапов.
- Использовать критерии приёмки и KPI для измеримого контроля качества.

**Для переговоров с C-Level:**

- Показывать трёхэтапную модель (Проектирование → Эксплуатация → Отчуждение) как гарантию передачи способности.
- Для CIO/CTO: архитектурные паттерны RAG, инференса и агентов как техническая основа КП.
...
### Суть предложения (SCQA) {: #method_scqa }
...
- **Вопрос для решения:** как внедрять и масштабировать ассистентов на стеке **корпоративный RAG‑контур**, **сервер инференса MOSEC** / **инференс на базе vLLM** и **агентный слой платформы**, и **как отчуждать** экспертизу и артефакты клиенту без потери управляемости.
- **Рекомендуемый ответ:** опереться на целевую операционную модель (роли, KPI, риски), поэтапный PoC → Pilot → Scale, комплект отчуждения (код, конфигурация, данные, модели, runbook, обучение) и блок комплаенса (152-ФЗ, приказ Роскомнадзора № 140, NIST AI RMF, guardrails), а также на единую промышленную наблюдаемость (трассировки, метрики, учёт токенов). Глобальные отчёты экосистемных вендоров задают **фон по темпу внедрения**, но **не** норму для резидентного КП без отдельной правовой и тарифной оценки. Закладывать **три оси гибрида:** резидентность и обработка ПДн, размещение вспомогательных моделей (эмбеддинг, реранк, гард), размещение основной LLM. При этом **собственная** инженерная практика Comindware (четыре репозитория экосистемы: RAG‑контур, MOSEC, vLLM, платформенный агент) отделяется от **открытых** отраслевых материалов.
...
| **Сентябрь 2025** | Отдельное согласие на ПДн | Согсие должно быть получено как отдельный документ, а не в составе договора |
...
Связь с юнит-экономикой и TCO при многошаговых агентских цепочках — в сопутствующем резюме _«[Оценка сайзинга, CapEx и OpEx для клиентов](./20260325-research-executive-sizing-ru.md)»_.
...
### Ключевые решения по ролям {: #method_c_role_decisions }
...
|-----------|--------|---------------|
| **Техническое** | Исходный код, конфигурации, модели, IaC | 18–36 мес. |
...
- Экономический эффект: **13 трлн руб.** к 2030 году (превышает предыдущие прогнозы 4.2-6.9 трлн)
