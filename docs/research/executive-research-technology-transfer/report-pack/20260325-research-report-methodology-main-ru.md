...
### Сводка по карте GenAI (ноябрь 2025; ограниченное извлечение) {: #method_genai_map_nov2025_metrics_extract }

Ниже — как элементы PDF **карты сегментов** (ноябрь 2025, материалы red_mad_robot) соотносятся с уже разобранными в комплекте блоками (IMARC, Ведомости, опрос CMO — без повторения). Сводка задаёт **прозрачность происхождения** и границы применимости цифр.
