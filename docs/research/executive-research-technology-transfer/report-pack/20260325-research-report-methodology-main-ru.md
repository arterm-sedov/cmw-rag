...
### Роль документа в комплекте {: #method_role_in_pack }
...
- **Рекомендуемый ответ:**
    ...
    - глобальные отчёты экосистемных вендоров задают **фон по темпу внедрения**, но **не** норму для резидентного КП без отдельной правовой и тарифной оценки. Закладывать **три измерения гибридного размещения:** резидентность и обработка ПДн, размещение вспомогательных моделей (эмбеддинг, реранк, гард), размещение основной LLM;
...
- **MOSEC** — фреймворк с Rust-вебслоем и Python-воркерами: динамическая пакетная обработка, поэтапные пайплайны, облачные практики (прогрев, graceful shutdown, метрики). **Меньший расход памяти** — комбинация эмбеддинг+реранкер+охранник укладывается в ~5 ГБ (против ~10–15 ГБ у vLLM), что критично для ограниченных GPU. Подробные замеры — в _«[Перерасход памяти vLLM](./20260325-research-report-sizing-economics-main-ru.md#sizing_vllm_overhead_inference)»_.
...
- **сервер инференса MOSEC** — управление процессом, YAML-реестр с проверенными конфигурациями, воркеры эмбеддинга/реранкера/охранника на **одном HTTP-порту** с OpenAI-совместимыми маршрутами. **Одна сетевая точка** — проще политики безопасности и сопровождение. CLI: установка, запуск, статус, остановка, тестирование.
- **инференс на базе vLLM** — жизненный цикл процессов (загрузка, проверки здоровья), pooling-режимы для эмбеддингов/скоринга, YAML-реестр, гибкий выбор чекпоинтов под нагрузку. Выгрузка KV-кэша через LMCache (vLLM v1).ы
...
**2. Вспомогательные модели:** эмбеддинг, реранг, контент-охранник, при необходимости слой маскирования/NER до LLM; часто совмещаются на одном унифицированном HTTP-сервисе (**MOSEC**) или распределяются по отдельным процессам (**vLLM** и др.) в зависимости от нагрузки и поддерживаемых форматов.
...
Конвейер **корпоративный RAG-контур** при отчуждении должен оставаться воспроизводимым: ingestion, чанкинг, эмбеддинги, LLM, реранкинг, выбор фреймворка, agentic-петля, контур оценки качества и защитные механизмы. Консолидированная разведка по NeuralDeep и паттернам @ai_archnadзор приведена в параграфе _«[Практики и архитектуры RAG](#method_rag_practices_neuraldeep_retrieval)»_; первичные ссылки — в параграфе _«[Источники](#method_sources)»_.
