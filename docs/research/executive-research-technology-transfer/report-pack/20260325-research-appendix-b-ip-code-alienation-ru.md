...
Ниже перечисленные продукты — **подсказки заказчику** для ускорения прототипирования и освоения практик разработки с ИИ; они **не** входят в коммерческую поставку референс-стека **корпоративный RAG-контур**, **сервер инференса на базе MOSEC/vLLM**, агентный слой **Comindware Platform** и **не** заменяют промышленный контур RAG и инференса, описанный в этом документе.
...
!!! warning "Для контуров с 152-ФЗ"
    ...
    Для контуров с **152-ФЗ** и суверенитетом данных **не принимайте** OpenCode Zen как дефолт без оценки: хостинг и политики обработки данных определяются провайдерами шлюза (в т.ч. юрисдикция США). 

    Бесплатные линейки на Zen могут иметь **ограниченный срок** и **особые условия использования данных** — см. официальный текст Zen.
...
Для C-Level **отчуждение данных** — это фиксация того, какие знания и цифровые активы переходят к заказчику, какие из них нужно хранить для воспроизводимости и аудита, а какие можно удалить без потери операционной способности.
...
Для C-Level **отчуждение моделей** — это не только передача права использовать модель, но и передача политики версий, лицензий, ограничений применения и ответственности за деградацию качества после смены модели или весов.
...
- **Перевод модели в эксплуатацию:** порядок перезагрузки, переключения и возврата к предыдущей версии должен быть формализован в runbook и согласован с требованиями по SLA.
...
- **Открытые веса и лицензия:** при self-hosted чекпойнтах (в т.ч. GigaChat-3.1 под MIT — [Хабр, Сбер](https://habr.com/ru/companies/sberbank/articles/1014146/)) в комплект передачи входят идентификаторы релиза (HF/GitVerse), текст лицензии, политика фиксации версий и регрессионные eval при смене весов.
...
Для C-Level **отчуждение инфраструктуры** — это не техническая операция, а момент перехода ответственности за работоспособность контура, его затраты, риски и постоянные цифровые активы.
...
- **Документация:** архитектура, runbook, регламенты эксплуатации, критерии приёмки и журнал ограничений.
...
Такая логика соответствует рыночной практике transfer readiness в BOT-моделях: смена владельца считается успешной, когда стабильны люди, процессы, документация, доступы и delivery rhythm, а не только подписан акт передачи ([InCommon — BOT Transfer Readiness & Handover Mechanics, 2025/2026](https://www.incommon.ai/blog/transfer-readiness/)).
...
Коммерческие **курсы по безопасности LLM** у третьих лиц (иллюстративный пример публичной программы — «Large Language Models Security» у [«Лаборатории Касперского»](https://www.kaspersky.com/about/press-releases/kaspersky-introduces-a-new-training-large-language-models-security)) могут дополнять подготовку ИБ и разработки заказчика; это **опция**, а не часть базовой поставки **корпоративный RAG-контур** / **сервер инференса на базе MOSEC/vLLM** / агентный слой **Comindware Platform** без отдельного соглашения.
