...
В контексте **передачи инфраструктуры** речь идёт не о «простом выключении контейнера», а о контролируемом переводе контура в состояние, которое заказчик может **принять, перенести или вывести из эксплуатации** без потери управляемости, данных и договорной ясности по зоне ответственности.

- **Контролируемая остановка сервисов инференса:** в комплект передачи входит runbook со штатной последовательностью остановки **сервера инференса MOSEC** или **инференса на базе vLLM**; смысл шага — исключить зависшие задания, спор о том, кто отвечает за активный контур, и скрытые расходы после даты передачи.
- **Контейнеры и окружение:** при развёртывании через Docker заказчику передают не только порядок запуска, но и порядок остановки, удаления и повторного развёртывания контейнеров и томов; это фиксирует границу между «контур ещё эксплуатирует интегратор» и «контур уже принят заказчиком».
- **Постоянные данные и ресурсы:** завершение процесса освобождает память GPU, но не удаляет автоматически постоянные артефакты — данные **ChromaDB**, журналы, кэши моделей и содержимое томов. Для них отдельно фиксируют решение: сохранить, архивировать, мигрировать или удалить по политике заказчика и требованиям ИБ.
- **Критерий завершённой передачи:** на момент передачи должны быть понятны владельцы сервисов, место хранения постоянных данных и процедура восстановления или очистки; без этого «остановка контейнера» сама по себе не считается завершённым отчуждением инфраструктуры.
