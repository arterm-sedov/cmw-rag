...
# Приложение B. Отчуждение ИС и кода (KT, IP, лицензии, приёмка) {: #app_b }

## Обзор {: #app_b_pack_overview }

Здесь представлены рекомендации о том, **как сделать внедрение корпоративного GenAI коммерциализируемым и переносимым**: это позволит заказчику получить не только работающий контур, но и **способность** его эксплуатировать и развивать без потери управляемости.

Материал помогает согласовать: что входит в комплект (код, конфигурации, данные, модели, регламент эксплуатации (runbook)), как учитываются **лицензии и IP**, какие **критерии приёмки** закрепить в договоре. **Сметные последствия** и TCO — в _«[Сайзинг и экономика](./20260325-research-report-sizing-economics-main-ru.md#sizing_pack_overview)»_.

## Как использовать в продажах и у руководства {: #app_b_how_to_use }
...
- **Снижение рисков передачи**: как избежать «магии поставщика» (bus factor), когда система работает только при участии интегратора.
...
- При переговорах по **суверенитету ИС**, **теневому GenAI** (маркетинг/личные аккаунты) и требованиям ИБ — показывать, что контур проектируется как **контролируемый** и **передаваемый** актив, а не «SaaS-черный ящик».
- Для выбора модели взаимодействия (управляемый сервис vs BOT vs create–transfer) — опираться на таблицу [«Модели поставки и передачи»](#app_b_delivery_transfer_models_ip_knowledge).

!!! warning "Ограничения"

    Не используйте отчёт как:

    - Полную смету без привязки к профилю нагрузки заказчика (TCO см. в параграфе _«[Роль документа в комплекте](./20260325-research-report-sizing-economics-main-ru.md#sizing_pack_overview)»_ отчёта «Сайзинг и экономика (CapEx / OpEx / TCO)»).
    - Юридическое заключение по IP без согласования с юристами заказчика.

**Минимально целостный комплект передачи (что фиксировать в договоре):**

См. [«Пакет отчуждения (минимально целостный)»](#app_b_alienation_package_minimal) и [«Критерии приёмки передачи (чек-лист)»](#app_b_transfer_acceptance_criteria_checklist). Рекомендуемый подход: закрепить артефакты передачи + критерии приёмки + срок **hypercare** + владельцев компонентов на стороне заказчика.
...
- **[OpenRouter](https://openrouter.ai/)** — агрегирующий **API-шлюз** к множеству зарубежных провайдеров; типичное применение — **IDE, coding agents, прототипирование** (в т.ч. совместимо с конфигурацией агентный слой **Comindware Platform** в upstream).
...
- **Cursor** — коммерческая IDE с подпиской; ориентиры по токенам для сравнения см. в параграфе _«[FinOps и юнит-экономика нагрузки](./20260325-research-report-sizing-economics-main-ru.md#sizing_finops_unit_economics)»_ отчёта «Сайзинг и экономика (CapEx / OpEx / TCO)».
...
**Питч «маркетинг / shadow SaaS / суверенитет ИС» (включать при активном движении продаж):** высокая концентрация на универсальных зарубежных сервисах усиливает риски **утечки данных** через неучтённые каналы, непрозрачных **обработчиков данных** и смешения **корпоративных активов** с личными учётными записями. В договоре и комплекте отчуждения полезно явно зафиксировать **реестр ИИ-инструментов**, **операторов/обработчиков**, политику логирования и запрет **теневого** GenAI вне согласованного контура; количественные доли и барьеры опроса — в _«[Российский рынок](./20260325-research-report-sizing-economics-main-ru.md#sizing_russian_market)»_.
...
- **Реестр доверенных моделей:** публикация открытых весов **не заменяет** проверку допуска модели для госсектора и КИИ (см. _Приложение D, параграф «[Управление рисками и соответствие (Compliance)](./20260325-research-appendix-d-security-observability-ru.md#app_d__risk_management_compliance)»_).
...
**5-фазная модель BOT для ИИ-проектов (актуальная 2025-2026):**
...
- Измерение ROI до развёртывания (только 16% масштабируются, 25% достигают ROI)
...
- Пройдены согласованные сценарии eval; зафиксированы базовые метрики.
...
- Цель: более **11 трлн руб.** влияния ИИ на ВВП к 2030 году ([Указ Президента №124](https://www.consultant.ru/document/cons_doc_LAW_473037/)).
- 7,7 млрд руб. на федеральный проект «ИИ» (2025).

**Тренд 2025-2026:**

- Массовое открытие офисов внедрения ИИ.
- Рост вакансий с ИИ-навыками: **+89%** в 2025; **+170%** YoY в I кв. 2026 к I кв. 2025 ([hh.ru × PR DEV](https://rb.ru/news/kolichestvo-vakansij-s-trebovaniem-k-vladeniyu-ii-vyroslo-vtroe-chashe-vsego-u-menedzherov-po-rabote-s-klientami/); _Приложение E «[Общая картина рынка GenAI (red_mad_robot)](./20260325-research-appendix-e-market-technical-signals-ru.md#app_e_rmr_market_map_2025)»_).
- 86% компаний, использующих генеративный ИИ, применяют дообучение открытых моделей.

## Методология отчуждения ИИ-активов {: #app_b_ai_assets_alienation_methodology }

При приобретении у инфраструктурного вендора **платформенного SKU** (управляемый LLM-шлюз, GPU как сервис, интеграционная платформа для агентов) в TCO заказчика обычно входят **лицензии или абонентская плата**, **сопровождение** (в т.ч. on-prem), **интеграции** с корпоративными системами и строки на обучение персонала. Чеклисты и комплект артефактов — в параграфе _«[Отчуждение обвязки](./20260325-research-report-methodology-main-ru.md#method_wrapper_detachment)»_ отчёта «Методология разработки и внедрения ИИ» и _Приложении D, параграф «[Пакет отчуждения: что добавить по наблюдаемости](./20260325-research-appendix-d-security-observability-ru.md#app_d__handoff_package_observability)»_; ниже в этом разделе — форматы передачи по компонентам.
...
**Экономика:**
- Выбор решения для команды разработки зависит от требований к суверенности данных:

### Облачные решения (SaaS) — данные обрабатываются outside

| Решение | Тариф | Стоимость (5–10 инженеров) | Notes |
|---------|-------|---------------------------|-------|
| **OpenCode Go** | $10/пользователь/мес | **~4 250–8 500 руб./мес** | Работает без VPN в РФ, open-source модели (GLM, Kimi, MiniMax), лучшее соотношение |
| **GitHub Copilot Business** | $19/пользователь/мес | **~8 000–17 000 руб./мес** | Глобальный стандарт, широкое enterprise-покрытие |
| **Cursor Teams** | $40/пользователь/мес | **~17 000–34 000 руб./мес** | Мощный IDE на базе VS Code |
| **Windsurf (Codeium)** | $40/пользователь/мес | **~17 000–34 000 руб./мес** | AI-first IDE |

### Гибридные решения — модель остаётся в контуре заказчика

| Решение | Модели | Стоимость | Особенности |
|---------|--------|-----------|-------------|
| **OpenCode Enterprise** | 75+ провайдеров (Ollama, vLLM, LM Studio) | Индивидуально | Поддержка локальных моделей, SSO, AI-шлюз, работает без VPN в РФ |
| **SourceCraft** | Yandex Cloud (GigaChat) | ~700 руб./пользователь/мес | Российское решение, Agent mode, code review |
| **GigaCode 2.0** | GigaChat | Бесплатно (ограничено) | Российское решение, Sber ecosystem |
| **Tabnine Enterprise** | Любые (self-hosted) | От $39/пользователь/мес | Полный on-prem, корпоративный контроль |
| **Sourcegraph Cody** | Любые (self-hosted) | От $59/пользователь/мес | Enterprise search + AI |

!!! note "Примечание"

    Полный on-prem (без облака) доступен только у Tabnine Enterprise и Sourcegraph Cody.
    
    Российские решения (SourceCraft, GigaCode) работают в облачном режиме. OpenCode Enterprise позволяет использовать собственные модели через API (в т.ч. локальные Ollama/vLLM).

### Локальные модели (on-prem-GPU) — полная инфраструктура

Диапазон **~127 500 – 212 500 руб./мес** соответствует стоимости выделенных GPU-серверов (A100/H100) с инфраструктурой — для команд, требующих полного контроля без внешних подключений.

- Qwen3-Coder-Next MoE: 24 ГБ VRAM, 64K+ контекст
- GLM-4.7-Flash MoE: 16-24 ГБ VRAM, 200K контекст

**Рекомендация для enterprise РФ:**

| Приоритет | Решение | Когда выбрать |
|-----------|----------|----------------|
| **1** | OpenCode Enterprise | Максимальная гибкость + работает без VPN + поддержка локальных моделей |
| **2** | SourceCraft (Yandex Cloud) | Всё в экосистеме Yandex, быстрое внедрение |
| **3** | GigaCode 2.0 (SberTech) | В экосистеме Сбера, бесплатный старт |
| **4** | Tabnine Enterprise | Требование полного on-prem без облака |
| **5** | Sourcegraph Cody | Требование enterprise search + AI |
...
1. **Для новых внедрений:**
   ...
   - Перейдите on-premise для продакшна
...
### Инструменты разработки с ИИ (ориентиры для заказчика, вне SKU **Comindware**) {: #app_b_ai_development_tools_customer_guidelines }
