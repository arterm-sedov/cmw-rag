...
## Практический смысл для ИБ и эксплуатации {: #app_d__how_to_use }

Используйте приложение как канонический источник по комплаенсу, периметру данных, наблюдаемости и контрольным требованиям перед промышленным запуском. Рыночный и продуктовый радар вынесен в _[Приложение E «Рыночные и технические сигналы»](./20260325-research-appendix-e-market-technical-signals-ru.md#app_e_root)_; здесь остаются только те сигналы, которые влияют на ИБ, контроль и эксплуатацию.
...
- Обязательные сигналы: токены, задержки, ошибки, глубина агентского цикла, события политик (защитные механизмы, эскалации).
...
- [Рынок РФ, наблюдаемость LLM и референс-стек Comindware (Phoenix)](#app_d__russia_llm_observability_phoenix_reference)
...
### Рынок РФ, наблюдаемость LLM и референс-стек Comindware {: #app_d__russia_llm_observability_phoenix_reference }
...
### Контекст-трекер и диагностика RAG {: #app_d__context_tracker_rag_diagnostics }
...
- **Связка с телеметрией:** даже при корректной архитектуре спанов организация может **записывать в журнал избыточно** из культуры «сохранить всё»; управленческий барьер «безопасность и данные» из того же исследования усиливает требование к **явной политике** сэмплирования и ретенции, согласованной с ДПО.

### Связь с контуром оценки качества {: #app_d__quality_eval_connection }
...
В _Приложении B «[Пакет отчуждения (минимально целостный)](./20260325-research-appendix-b-ip-code-alienation-ru.md#app_b_alienation_package_minimal)»_ к базовому комплекту передачи целесообразно добавить строки по телеметрии: владельца архитектуры наблюдаемости, перечень бэкендов, политику **сэмплирования** (в т.ч. полные трассы для ошибок / канареек), **ретенцию**, эксплуатационный регламент разбора инцидента по `trace_id` и правила маскирования ПДн.
...
## Агенты, инструменты, память и наблюдаемость {: #app_d__agents_tools_memory_observability }
...
#### Продуктовый радар: смежные технологии {: #app_d__product_radar_handoff }

Краткие обзоры **GraphOS**, **Nested Learning**, **LEANN** и **Perplexica** вынесены в _«[Приложение E. Рыночные и технические сигналы](./20260325-research-appendix-e-market-technical-signals-ru.md#app_e_architecture_product_radar)»_. В этом приложении сохраняются только их последствия для контроля: глубина агентского цикла, изоляция среды, требования к телеметрии, долговременной памяти и минимизации данных.

#### Защитные механизмы: Архитектурный паттерн №1 для эпохи AI-агентов {: #app_d__guardrails_ai_agent_pattern }
...
### Международный регуляторный контекст: EU AI Act {: #app_d__eu_ai_act_context }
...
4. **Уязвимости цепочки поставок агентов** (Agentic Supply Chain Vulnerabilities, ASI04) — компрометация сторонних модулей, промптов, навыков, MCP-серверов и иных артефактов среды исполнения; иллюстративный кейс LiteLLM/Telnyx вынесен в раздел _«[Источники: инцидент LiteLLM и Telnyx](#app_d__litellm_telnyx_incident_2026)»_.
...
!!! warning "Справочно: инцидент LiteLLM и Telnyx (март 2026)"

    Группировка TeamPCP скомпрометировала PyPI-пакеты LiteLLM и Telnyx SDK; ключевой практический вывод для агентских контуров — традиционные сигнатурные SCA-проверки недостаточны без **поведенческого анализа зависимостей** и **строгого контроля исходящего трафика**. Детали и источники сведены в конце приложения: _«[Инцидент LiteLLM и Telnyx (март 2026): цепочка поставок](#app_d__litellm_telnyx_incident_2026)»_.
...
### AI TRiSM и рамки доверия {: #app_d__ai_trism_trust_frameworks }
...
В марте 2026 года группировка TeamPCP скомпрометировала пакеты **LiteLLM** и **Telnyx SDK** в PyPI. По открытым разборам, вредоносный код в одном случае активировался через `.pth`-механику при импорте, а в другом использовал стеганографию в WAV-файле, что усложняло обнаружение типовыми SCA-практиками. Практический смысл для агентных и LLM-шлюзов: недостаточно только сигнатурного сканирования зависимостей, нужны поведенческий анализ, жёсткий контроль исходящего трафика и отдельный мониторинг цепочки поставок Python-пакетов. Этот кейс в документе используется как **иллюстрация** класса **Agentic Supply Chain Vulnerabilities**, а не как самостоятельная таксономия рисков.
