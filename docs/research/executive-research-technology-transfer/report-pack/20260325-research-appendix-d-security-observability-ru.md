...
Здесь дан **воспроизводимый каркас** для руководства и архитектуры: какие сигналы собирать, как связать их с качеством и FinOps и что учитывать в РФ при ПДн.

Раздел опирается на **открытые спецификации**, дополняет паттерны агентов и не привязан к единственному коммерческому продукту.

### Задача для бизнеса и эксплуатации {: #app_d_business_ops_objective }

**Без сквозной видимости** по шагам пайплайна (поиск, вызов модели, инструменты агента) инциденты и регрессии качества разбираются медленнее. Без метрик токенов и задержек невозможно устойчиво связывать рост нагрузки с P&L. Для агентов критичны также **глубина цикла** (число итераций) и корреляция с _«[OWASP Top 10 for Agentic Applications](https://genai.owasp.org/resource/owasp-top-10-for-agentic-applications-for-2026/)»_.

**Для разработки с агентами:** отдельно учитывайте **циклы разработки** (план → реализация → проверка → итерация) — суммарные токены, длительность и число циклов стыкуются с OpEx инженерии; ориентиры — в _«[FinOps и юнит-экономика нагрузки](./20260325-research-report-sizing-economics-main-ru.md#sizing_finops_unit_economics)»_.

**Долгосрочная агентная память** (General Agentic Memory, GAM) — исследовательский класс архитектур, увеличивающих число шагов и требующих аудита записей в долговременное хранилище. Риск **Memory and Context Poisoning** (OWASP Agentic 2026) остаётся актуальным. Такие схемы приводятся как **ориентир НИОКР**, не как обязательная поставка.
