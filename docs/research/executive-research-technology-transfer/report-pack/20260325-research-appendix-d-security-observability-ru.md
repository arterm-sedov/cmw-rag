...
Приложение задаёт **периметр контроля** для промышленного GenAI: модель угроз, комплаенс (в т.ч. ПДн, 152-ФЗ), промышленная **наблюдаемость (observability)** и практические паттерны для RAG и агентов.

Основные отчёты дают **управленческие выводы**; здесь — детализация для **решений по ИБ и эксплуатации**.
...
В открытом разборе **МТС** описан рабочий контур ассистента для саппорта на базе **Confluence** и **Jira**: **гибридный поиск** (pgvector + BM25), **локальные эмбеддеры** (в материале — BGE-m3) без обращения к внешнему API эмбеддеров, нормализация текста, чанкинг порядка **~3000** символов с перекрытием, раздельные конвейеры **индексации** и **поиска** с общей базой знаний. Это переносимый ориентир для класса **Hybrid Search RAG** и **Agentic RAG (локальный контур)** на референс-стеке **корпоративный RAG-контур** / **сервер инференса MOSEC**, независимо от выбора облака ([Хабр, МТС](https://habr.com/ru/companies/ru_mts/articles/970476/); смежная публикация — [Хабр, МТС](https://habr.com/ru/companies/ru_mts/articles/970392/)).
