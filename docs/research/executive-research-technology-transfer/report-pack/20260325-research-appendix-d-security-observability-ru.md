...
**Трансграничные шлюзы разработки** (OpenRouter, OpenCode Zen и аналоги) для **продакшн**-решений с ПДн в РФ **не подменяют** маркировку, локализацию и логику реестра — см. в этом документе «Ориентиры для заказчика: инструменты ускорения разработки» и сопутствующее резюме **Оценка сайзинга, CapEx и OpEx для клиентов** (экономика таких шлюзов).
