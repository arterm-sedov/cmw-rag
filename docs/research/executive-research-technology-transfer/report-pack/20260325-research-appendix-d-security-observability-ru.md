...
- Показывайте, что GenAI-контур — это **управляемый актив** с измеримыми метриками безопасности, а не «чёрный ящик».
- Для CISO: опирайтесь на OWASP ТОП-10 для приложений с LLM (2025), OWASP ТОП-10 для агентских приложений (2026) и NIST AI RMF как методологические ориентиры.
- Для переговоров по суверенности: размещение данных и резидентность — см. _«[Применимость в России: что не блокируется и где нужны оговорки](#app_d__russia_applicability_caveats)»_; для госсектора и КИИ — _«[Госсектор, КИИ, ФСБ/ФСТЭК и реестры](#app_d__ai_bill_gov_kii_registry)»_.

Ниже даны перечни **ТОП-10 OWASP** для приложений с LLM и для агентских приложений — с кодом категории и кратким пояснением.
...
!!! warning "Ограничения"
    ...
    - Полную модель угроз **вместо** отдельного анализа и адаптации под профиль заказчика.
...
- Для ПДн по умолчанию применяйте **минимизацию**: не записывайте полный контент в телеметрию; храните контент отдельно с контролем доступа (см. раздел _«[Персональные данные и содержимое в телеметрии (152-ФЗ)](#app_d__personal_data_telemetry_152fz)»_).
...
- Для «недоверенного исполнения» (код/широкие инструменты) — выделите отдельный контур изоляции и политики доступа.

**4. Рамки риска (ориентиры):**

- [OWASP ТОП-10 для приложений с LLM (2025)](https://genai.owasp.org/resource/owasp-top-10-for-llm-applications-2025/) и [OWASP ТОП-10 для агентских приложений (2026)](https://genai.owasp.org/resource/owasp-top-10-for-agentic-applications-for-2026/) используйте как чек‑лист классов рисков, не подменяя ими внутреннюю модель угроз заказчика. См. анализ рисков в этом документе: [OWASP ТОП-10 для приложений с LLM (2025)](#app_d__owasp_top_10_llm_2025), [OWASP ТОП-10 для агентских приложений (2026)](#app_d__owasp_top_10_agentic_2026).
...
Ориентиры **токенов/с** из публикаций сообщества и обзоров полезны лишь для грубой прикидки до замеров; в эксплуатации сопоставляйте их с метриками времени генерации токенов на целевом стенде. Иллюстративные таблицы по локальному железу и MoE — в _«[Отчёт. Сайзинг и экономика](./20260325-research-report-sizing-economics-main-ru.md)»_.
...
Ниже — паттерны, влияющие на стоимость контекста, выбор CLI vs толстых протоколов, память агента и наблюдаемость в продакшне; при поставке **корпоративный RAG-контур** / агентный слой **Comindware Platform** они сверяются с целевой операционной моделью и комплектом отчуждения, а не принимаются как обязательный «золотой стек».
...
Помимо технических угроз и комплаенса по ПДн, внедрение ассистентов и агентов упирается в **человеческий контур**: недоверие к ответам без **eval** и цитат; **неясная модель допустимого использования** (что можно спрашивать, какие данные смешивать); ожидание **гарантированного эффекта** без baseline; страх **ошибки** и страх **потери контроля** при автономии. Эти факторы усиливают запрос на **human-in-the-loop**, поэтапное расширение полномочий инструментов, **обучение** и коммуникацию политики — в связке с отчётом _«[Методология — Стратегия внедрения ИИ и организационная зрелость](./20260325-research-report-methodology-main-ru.md#method_ai_strategy_org_maturity)»_ и с [OWASP ТОП-10 для приложений с LLM (2025)](#app_d__owasp_top_10_llm_2025) / [OWASP ТОП-10 для агентских приложений (2026)](#app_d__owasp_top_10_agentic_2026) в разделе [Как использовать в продажах и у руководства](#app_d__how_to_use). Подраздел **не** подменяет юридическую квалификацию и **не** дублирует экономическую строку «**Теневые ИИ-инструменты**» в отчёте по сайзингу.
...
!!! warning "Важно: статус законопроекта"
    ...
    Проект федерального закона о госрегулировании ИИ (внесение — Правительство РФ, обсуждение в **марте 2026**) — **черновик**, не вступил в силу.
...
- **До принятия и вступления в силу** конкретные обязанности нельзя трактовать как действующее право; **используйте** материалы проекта как **сигнал для дорожной карты** продуктовой и архитектурной готовности.
- **Закладывайте** готовность к **маркировке**, **уведомлениям** пользователей, возможному **реестру доверенных моделей** и **риск-ориентированным** требованиям.
- Для **госсектора** и **КИИ** **опирайтесь** на **действующие** требования **ФСБ**, **ФСТЭК** и реестров на **дату внедрения**; проект закона об ИИ эту базу **не заменяет**.
...
[OWASP AI Testing Guide](https://github.com/OWASP/www-project-ai-testing-guide) задаёт рамку **проверки доверия** к ИИ-системам (атаки, методы тестирования, увязка с зарождающимися стандартами). Используйте его **наряду** с, а не вместо проверок **HTTP/API-обвязки** ассистента.
...
**Чат против агента:** обход ограничений в **диалоге** (jailbreak) и компрометация **агента с инструментами** — разные классы последствий: второй вариант масштабирует ущерб через API, файловые системы, биллинг токенов и интеграции. Здесь опираемся на [OWASP ТОП-10 для агентских приложений (2026)](https://genai.owasp.org/resource/owasp-top-10-for-agentic-applications-for-2026/) и внешние разборы вроде [Kaspersky — agentic risks / ASI](https://www.kaspersky.com/blog/top-agentic-ai-risks-2026/29988/).
