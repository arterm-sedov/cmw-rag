...
| Контур оценки качества | Наборы тестов, критерии, метрики и регрессионные проверки, которые позволяют отслеживать деградацию после изменений модели, индекса или промпта. |
...
- Как обеспечить security, комплаенс и промышленную observability (контур контроля, data minimization posture)? → [Приложение D. Безопасность, комплаенс и observability](./20260325-research-appendix-d-security-observability-ru.md)
- Как проектировать изоляцию и сеть для агентского исполнения (граница доверия, egress, краткоживущие учётные данные)? → [Приложение D. <б>езопасность, комплаенс и observability](./20260325-research-appendix-d-security-observability-ru.md#app_d__trust_boundary_agent_environment)
