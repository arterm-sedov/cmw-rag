...
| **Вайб-кодинг в Comindware Platform** | Разработка на платформе требует глубокого знания API | Естественный язык → LLM-классификация намерения → детерминированные JSON-операции платформы (Navigate, UserCommand, Chart) | Аналитики формулируют намерение; агент исполняет детерминированно без ручного ввода |
