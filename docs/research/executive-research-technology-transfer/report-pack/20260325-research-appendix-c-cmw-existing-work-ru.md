...
Критерии приёмки при передаче — в _Приложении B «[Отчуждение ИС и кода (KT, IP, лицензии, приёмка)](./20260325-research-appendix-b-ip-code-alienation-ru.md#app_b_pack_overview)»_; модель внедрения и экономика — в двух основных отчётах.
...
- **Comindware** поставляет не только внедрение, но и **передаваемые артефакты** (код/конфигурации/эксплуатационный регламент/контур оценки качества/обучение) — см. _Приложении B «[Отчуждение ИС и кода (KT, IP, лицензии, приёмка)](./20260325-research-appendix-b-ip-code-alienation-ru.md#app_b_pack_overview)»_.
...
- **Наблюдаемость LLM (референс):** слой **Arize Phoenix** + **OpenInference** для трасс GenAI и связки с офлайн/онлайн оценками при **self-hosted** размещении в контуре заказчика или облаке РФ — _Приложение D, параграф «[Рынок РФ, наблюдаемость LLM и референс-стек Comindware](./20260325-research-appendix-d-security-observability-ru.md#app_d__russia_llm_observability_phoenix_reference)»_.
...
| **Сервер инференса на базе MOSEC** | Обвязка **Comindware** вокруг **Mosec** для единого HTTP-сервиса специализированных моделей | Эмбеддинг, реранк, защитные модели, единая точка доступа и конфигурации |
| **Сервер инференса на базе vLLM** | Обвязка **Comindware** вокруг **vLLM** для промышленного инференса LLM и части pooling-моделей | LLM, OpenAI-совместимый API, KV-кэш, пакетная обработка, вызов инструментов, проверки доступности |
| **Агентный слой Comindware Platform** | AI-агент для управления **Comindware Platform** | 49 инструментов (27 **Comindware** + 22 утилиты) |
| **Слой наблюдаемости LLM (Phoenix)** | Трассировка GenAI, дашборды, эксперименты и оценка качества в контуре заказчика | OpenTelemetry, OpenInference, Arize Phoenix (self-hosted) |
...
В референс-стеке **агентный слой Comindware Platform** опирается на **краткосрочную** память диалога в стеке LangChain и на **корпоративный RAG-контур** для извлечения из индекса. Исследовательские подходы к долговременной агентной памяти относятся к повестке развития, а не к текущему составу поставки.
