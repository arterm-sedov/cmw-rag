...
### Рынок AI: статистика a16z (март 2026) {: #sizing_ai_market_a16z_stats }
...
**Источник:** [_a16z Top 100 AI Apps 6th Edition_](https://a16z.com/100-gen-ai-apps-6/)

### Распределение глобального рынка (веб-трафик, январь 2026) {: #sizing_market_distribution }

| Модель | Трафик vs ChatGPT | Оценка доли | Комментарий |
|--------|-------------------|-------------|-------------|
| ChatGPT | 1,0x (база) | ~100% | 900 млн еженедельных активных пользователей |
| Gemini | ~0,37x | ~37% | #2 по трафику, рост платных подписчиков +258% г/г |
| DeepSeek | ~0,35x | ~35% | #3 по трафику, 316 млн визитов/мес (янв 2025) |
| Claude | ~0,033x | ~3,3% | #4, рост платных подписчиков +200% г/г |
| Kimi (Moonshot) | ~0,04x | ~4% | ~36 млн MAU (окт 2025), китайский «ChatGPT» |
| Qwen (Alibaba) | ~0,12x | ~12% | >180 млн MAU в Qwen Chat + 700+ млн загрузок на HF (март 2026) |

**Примечание:** Qwen и Kimi — преимущественно API/developer-модели, не представленные в a16z Top 100 (ориентирован на потребительские веб-приложения). Их значимость — в экосистеме разработчиков и корпоративном API.
...
**Для КП:** используйте эту информацию для контекста о трендах, но бюджетные расчёты стройте на российских тарифах (см. _«[Тарифы и провайдеры РФ](#sizing_russian_ai_cloud_tariffs)»_).

### География использования ИИ {: #sizing_ai_usage_geography }
...
!!! note "Разрыв между созданием и использованием ИИ"
...
- **Китайский подход:** массовое внедрение ИИ в экономику — см. _«[AI + Economy: китайская модель](https://t.me/Redmadnews/5159)»_
...
**Ключевые метрики:**
- CAGR (весь AI): **26,5%** (IMARC, 2025–2033)
- CAGR (GenAI): **19,04%** (IMARC, 2025–2033)
- GenAI составляет ~11% от всего AI-рынка в 2025 году (58/533)
...
**Ограничения данных по российскому рынку:** ряд западных аналитических агентств (Gartner, IDC, Statista) прекратили публикацию отдельных отчётов по российскому рынку после 2022 г. Часть приведённых цифр по рынку РФ основана на ограниченном объёме данных из российских источников (Коммерсантъ, ВЦИОМ, Yakov & Partners, NTI/MIPT).

**Источники:** ВЦИОМ, Коммерсантъ, Digital Budget, Smart Ranking, CNews, IMARC, NTI/MIPT. Оценку **экономического эффекта** ИИ к 2030 году (в трлн руб.) см. отдельно в _«[Методологии разработки и внедрения ИИ](./20260325-research-report-methodology-main-ru.md#method_economic_effect)»_.

!!! note "Разные методологии"
...
### Агентный код-ревью (на примере Claude Code Review) {: #sizing_claude_code_review }
...
### Российские модели {: #sizing_russian_models_march_2026 }
...
### Калькуляция расхода и стоимости токенов по классам задач {: #sizing_token_calculation_classes }

Для оценки токенов при планировании бюджета используйте классификаторы по агентам и данным.

#### Классы агентов по длине системного промпта {: #sizing_agent_class_system_prompt_length }
...
#### Классы данных по длине пользовательского текста {: #sizing_data_class_user_text_length }
...
!!! tip "Квантование, MoE и плотные модели"

    Квантование Q4 снижает расход VRAM примерно в 4 раза с минимальной потерей качества. Это не единственный вариант квантования, его следует побирать экспериментально под ваши задачи.
    
    Модели MoE (Mixture-of-Experts) имеют высокое общее число параметров, но используют лишь небольшую их часть (sparse activation), что даёт высокую скорость инференса при относительно скромном потреблении памяти по сравнению с плотными (dense) моделями аналогичного качества.
...
#### Перерасход памяти вспомогательных моделей {: #sizing_vllm_overhead_inference }
...
| :--- | :--- | :--- | :--- |
| **Эмбеддер** | | | | |
| | `ai-forever/FRIDA` | 3,6 ГБ | ~2,1 ГБ |
| | `Qwen3-Embedding-0,6B` | 1,9 ГБ | ~1,1 ГБ |
| | `Qwen3-Embedding-4B` | 8,9 ГБ | ~3,0 ГБ |
| **Ранжировщик** | | | |
| | `DiTy/cross-encoder` | 2,3 ГБ | ~1,4 ГБ |
| | `Qwen3-Reranker-0,6B` | 1,5 ГБ | ~0,9 ГБ |
| | `Qwen3-Reranker-4B` | 8,1 ГБ | ~2,8 ГБ |
| **Защитник** | | | |
| | `Qwen3Guard-Gen-0,6B` | 1,8 ГБ | ~1,0 ГБ |
| | `Qwen3Guard-Gen-4B` | 8,8 ГБ | ~3,0 ГБ |
| **Комбинация (0,6B)** | Embed+Rerank+Guard | ~5,2 ГБ | ~3,0 ГБ |
...
| Embed 4B + Rerank 4B | ~17 ГБ | ~11 ГБ | ~7 ГБ | ⚠️ Тесно |
| **Любая модель 8B** | ~16–18 ГБ | ~10–12 ГБ | <8 ГБ | ⚠️ Риск OOM |
...
## Альтернативный инференс: edge-устройства, потребительское железо {: #sizing_local_edge_inference .pageBreakBefore }
...
### Кейс: Qwen3.5-397B на M3 Max 48 ГБ {: #sizing_community_qwen_consumer_hardware }
...
**Источник:** [LLM под капотом](https://t.me/llm_under_hood) (Dan Woods extract 2026-03-18, MLX/"LLM in a Flash").

**Метод:** Claude Code / OpenCode + auto-research + оптимизация памяти (квантованный MoE, эквивалент ≈72B, работает на 48 ГБ).
...
- **Корпоративные R&D-кластеры:** кластеризация Mac Studio (M3 Ultra до 256 ГБ RAM) для инференса 70B+ моделей — альтернатива недоступным или дорогим NVIDIA-кластерам. См. [Apple specs](https://www.apple.com/mac-studio/specs/).
...
- **РФ:** 5–7 ₽/кВт·ч (коммерческие тарифы для бизнес-потребителей на апрель 2026; в зарубежных ЦОД — ~8,5 руб./кВт·ч)
...
!!! note "Ключевые академические публикации по оптимизации инференса"
