...
### Суть предложения (SCQA) {: #sizing_scqa }
...
- **Рекомендуемый ответ:**
    - начинать с облачного PoC по российским тарифам, затем при стабильной утилизации **>60 %** оценивать безубыточность on‑prem; опираться на дерево факторов стоимости, сценарный сайзинг и раздел TCO ниже, включая **строки наблюдаемости** (телеметрия, ретенция, сэмплинг) как часть OpEx и **строки пре‑LLM** (CPU/GPU для вспомогательных моделей, хранилище плейсхолдеров, регрессионные тесты при смене бэкенда);
...
#### Барьеры и эффекты внедрения в РФ (CMO Club, 2025)

- **Барьеры (качество и безопасность):** **43%** отмечают галлюцинации и ошибки; **отдельно 43%** указывают **риски утечки данных** (это разные группы опасений).
- **Эффекты (производительность):** **43%** опрошенных руководителей отмечают реальное **снижение нагрузки на команды** уже на этапе пилотов.
...
!!! note "Примечание: различие цен Cloud.ru и SberCloud"
    ...
    Цена 12,2 ₽/млн на Cloud.ru относится к Evolution Foundation Models API (управляемый сервис токенов). Цена ~65 ₽/млн на SberCloud — к GigaChat API с другими условиями пакетов. Для КП сверять актуальный прайс провайдера.
...
- Барьеры: **53%** — необходимость доработки контента; **49%** — шаблонность; **43%** — галлюцинации и ошибки; **отдельно 43%** — риски утечки данных (_примечание: источник опроса CMO Club не удалось верифицировать напрямую; цифра приводится по вторичным материалам_).
...
|:---|---:|:---|
| Эконом | 12–65 | GigaChat 3.1 Lightning (Cloud.ru, SberCloud) |
