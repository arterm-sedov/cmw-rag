...
**Валюта:** Курс USD/RUB для иллюстративных ориентиров — см. [приложение A](./20260325-research-appendix-a-index-ru.md#app_a_fx_policy).
...
### Локальные модели для кодинга и снижения затрат {: #app_e_local_models_coding_cost_reduction }
...
| **IBM Granite-20B-Code** | 12-16 ГБ | 8K | Корпоративные требования комплаенса (Java, C#, SQL) |
...
### Агенты для разработки (IDE/CLI-агенты) {: #app_e_coding_agents }
...
_Примечание: первоисточник опроса CMO Club не удалось верифицировать напрямую; цифры приводятся по вторичным материалам (Telegram-пост, RB.RU)._
...
### Обвязка разработки (llm_under_hood) {: #app_e_development_wrapper_llm_under_hood }
...
**Сравнение с облачными подходами**

| Подход | Доступность | Применение |
|--------|-------------|------------|
| LLM in a Flash / mlx-flash | Только Apple Silicon (local) | Edge, Mac-based on-prem |
| AWS LMCache | AWS SageMaker | Облачный multi-tier KV-caching (GPU→CPU→disk) |
| Gemini 2.5 Flash-Lite | Google Cloud | Облачная модель с оптимизированной латентностью |
