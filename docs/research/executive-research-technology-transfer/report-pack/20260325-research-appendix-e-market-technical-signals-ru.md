...
**Сигнал:** потребительское железо для enterprise-моделей. Демонстрирует возможность запуска крупных MoE-моделей на потребительском железе через оптимизацию памяти и квантование. Это даёт возможность построения суверенных агентов на стеке **Comindware**.
...
### Инструменты дообучения {: #app_e_unsloth_studio }

**Сигнал:** дмократизация fine-tuning. No-code-инструменты снижают порог входа в дообучение открытых моделей, что ускоряет цикл экспериментов и снижает затраты на GPU-ресурсы.

**Unsloth Studio** — no-code-платформа для дообучения и инференса LLM. Входит в экосистему инструментов, которые сокращают порог входа в fine-tuning и ускоряют итерации при работе с открытыми моделями.
...
**Сигнал:** edge-AI и тонкие клиенты. Демонстрирует возможность запуска агентов на минимальном железе с подключением к облачной LLM через прокси.
...
## Рынок ИИ: глобальная статистика {: #app_e_ai_market_a16z_stats }
...
### Распределение рынка приложений {: #app_e_market_distribution }
