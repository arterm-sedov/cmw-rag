...
### Модели для локального инференса {: #app_e_local_models_coding_cost_reduction }

Замена подписки на облачный **агент для разработки** (~127 500 – 212 500 руб./мес для 5–10 инженеров) на локальные модели:
...
**Стратегия развертывания:** «мозг + периферия (edge)» (тяжёлая модель на сервере для архитектурного ревью + микро‑модель локально для мгновенного автокомплита)
...
Дайджест глобальных продуктовых и инфраструктурных сигналов. Обоснование CapEx/OpEx и SLA — через дерево факторов стоимости, сценарный сайзинг и российские тарифы в основном отчёте по экономике.
...
**Метод:** Claude Code / OpenCode + autoresearch + "LLM in a Flash" (Apple)
...
**Примечание:** США создали большинство AI‑продуктов, но по уровню использования находятся примерно на 20‑м месте.
...
Дополнение к разделу «Локальный инференс». Детали кейсов Qwen на M3 Max, Picoclaw и сравнения CLI vs MCP — см. в основном отчёте по сайзингу.
...
### Перспективные технологии edge-инференса (2024–2026) {: #app_e_prospective_edge_inference }

!!! warning "Ограниченная доступность"

    Технологии данного раздела применимы **только к локальному инференсу на Apple Silicon** и не доступны в облачных GPU (AWS, Azure, GCP, Cloud.ru). Используйте для оценки перспектив on-prem на Mac-инфраструктуре, а не для планирования облачного бюджета.
...
- **Реализация 2025–2026:** проект `mlx-flash` (март 2026) — практическая реализация для MLX; возможность запуска 30B+ моделей на 16GB Mac, 70B+ на 32GB+
- **Реальный кейс (2026):** CVS Health запустила Qwen 3.5 397B на MacBook Pro 48GB, используя ~5.5GB RAM

**Вывод для бюджетирования:** при планировании облачного инференса ориентируйтесь на стандартные тарифы GPU (см. основной отчёт), а не на edge-оптимизации. Edge-техники применяйте только при обосновании локального Mac-based развёртывания.

**Сравнение подходов к оптимизации инференса**

| Подход | Доступность | Применение |
|--------|-------------|------------|
| LLM in a Flash / mlx-flash | Apple Silicon (локально) | Edge-кейсы, автономные станции в РФ (без VPN) |
...
| vLLM / MOSEC | Свободное ПО | Продакшн-инференс на собственных мощностях (Comindware) |
