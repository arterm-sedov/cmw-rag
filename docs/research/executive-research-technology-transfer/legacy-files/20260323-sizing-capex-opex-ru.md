...
**Проекты:** **корпоративный RAG-контур**, **сервер инференса MOSEC**, **инференс на базе vLLM**, **агентный слой платформы (Comindware Platform)**
**Дата:** 2026-03-24
...
В данном документе представлен подробный анализ аппаратных требований (сайзинга), капитальных затрат (CapEx) и операционных затрат (OpEx) для развертывания экосистемы ИИ Comindware с фокусом на российский рынок и локальные облачные провайдеры. Анализ основан на фактических измерениях VRAM из **сервер инференса MOSEC**, спецификациях vLLM из **инференс на базе vLLM** и тарифах российских облачных платформ (Cloud.ru, Yandex Cloud, SberCloud, Selectel Foundation Models Catalog, MWS GPT); при планировании ёмкости учитывайте нагрузку со стороны **корпоративный RAG-контур** (RAG) и **агентный слой платформы (Comindware Platform)** (агентные сценарии платформы).
...
### Инструменты разработки с ИИ (ориентиры, вне SKU Comindware)
