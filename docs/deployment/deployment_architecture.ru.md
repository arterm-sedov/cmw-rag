---
title: Архитектура развёртывания
tags: [deployment, cmw-rag, architecture]
hide: tags
---

# Хост {: #deployment_architecture_host }

## Запущенные сервисы {: #deployment_architecture_host_running_services }
...
## Незапущенные сервисы {: #deployment_architecture_host_stopped_services }

Отсутствуют — все основные сервисы активны.

---

# Топология репозиториев {: #deployment_architecture_repo_topology .pageBreakBefore }
...
## Граф зависимостей {: #deployment_architecture_repo_dependency_graph }
...
## Схема соединений {: #deployment_architecture_repo_connection_schema }
...
## Краткая таблица управления сервисами {: #deployment_architecture_repo_service_control }
...
# CMW-Mosec (порт 7998) {: #deployment_architecture_mosec .pageBreakBefore }
...
## Маршруты {: #deployment_architecture_mosec_routes }
...
## Переменные среды активных моделей {: #deployment_architecture_mosec_active_env }
...
## Запуск {: #deployment_architecture_mosec_startup }
...
## Альтернативные модели {: #deployment_architecture_mosec_alternative_models }
...
# ChromaDB (порт 8000) {: #deployment_architecture_chromadb .pageBreakBefore }
...
## Пуск / Стоп / Статус {: #deployment_architecture_chromadb_control }
...
## Файл сервиса {: #deployment_architecture_chromadb_service_file }
...
## Активная коллекция {: #deployment_architecture_chromadb_active_collection }

`mkdocs_kb_qwen06b_linux_qwen_default_instruct_chunk_768` (1024 измерения, соответствует Qwen3-Embedding-0.6B).

## Версионированные коллекции {: #deployment_architecture_chromadb_versioned_collections }
...
# RAG Gradio UI (порт 7860) {: #deployment_architecture_rag_gradio_ui .pageBreakBefore }

Точка входа: `rag_engine/api/app.py`.

Репозиторий: `github.com/cmw-team/cmw-rag` (pushurl: `arterm-sedov`).

## Два агента {: #deployment_architecture_rag_agents }
...
## Пуск / Стоп / Статус {: #deployment_architecture_rag_control }
...
## Файл сервиса {: #deployment_architecture_rag_service_file }
...
## Схема связей {: #deployment_architecture_rag_connection_schema }
...
## Переменные среды {: #deployment_architecture_rag_env }
...
# Интеграция с CMW Platform {: #deployment_architecture_cmw_platform .pageBreakBefore }
...
## Два экземпляра {: #deployment_architecture_cmw_instances }
...
## Основной: конвейер обработки заявки {: #deployment_architecture_cmw_support_pipeline }
...
## Второй / Лукойл: резюме документов {: #deployment_architecture_cmw_lukoil_summary .pageBreakBefore }
...
## Pydantic-схемы {: #deployment_architecture_cmw_pydantic_schemas }
...
## Category Enum {: #deployment_architecture_cmw_category_enum }
...
# Синхронизация корпуса (MkDocs) {: #deployment_architecture_corpus_sync .pageBreakBefore }

Синхронизирует документацию из внешнего MkDocs-репозитория в ChromaDB для индексации RAG.

## Systemd-таймер {: #deployment_architecture_corpus_systemd_timer }
...
## Установка таймера {: #deployment_architecture_corpus_timer_install }
...
## Внешний репозиторий {: #deployment_architecture_corpus_external_repo }
...
## Версии корпуса {: #deployment_architecture_corpus_versions }
...
## Ручной запуск {: #deployment_architecture_corpus_manual_run }
...
# Инструкция по восстановлению {: #deployment_architecture_recovery .pageBreakBefore }
...
## Требования к GPU {: #deployment_architecture_recovery_gpu_requirements }
...
> **Несоответствие версий драйвера:** на хосте установлены два пакета
> nvidia-driver (590 — модуль ядра, 595 — пользовательские библиотеки).
> PyTorch выводит `"Can't initialize NVML"`, но это **некритично** —
> CUDA-вычисления и инференс работают корректно.
...
# Соседние репозитории {: #deployment_architecture_neighbor_repos .pageBreakBefore }

## cmw-mosec {: #deployment_architecture_neighbor_mosec }
...
## cmw-vllm {: #deployment_architecture_neighbor_vllm }
...
# Заметки {: #deployment_architecture_notes }
