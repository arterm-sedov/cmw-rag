...
def search_kbid(kbid: str, show_content: bool = False, limit: int | None = None) -> None:
    ...
    res = store.collection.get(
        where=where_filter, include=include_params, limit=limit if limit is not None else 10000
    )
    ...
    for i, (doc_id, doc_content, meta) in enumerate(zip(ids, documents, metadatas), 1):
        ...
        other_meta = {
            ...
            if k not in ["kbId", "source_file", "chunk_index", "doc_stable_id"]
        }
        if other_meta:
...
Examples:
  ...
  python search_kbid.py 4578 --limit 10
    ...
    parser.add_argument("kbid", type=str, help="The kbid to search for")
    parser.add_argument(
        "--show-content", action="store_true", help="Display document content previews"
    )
    parser.add_argument(
        "--limit",
        type=int,
    ...
    search_kbid(kbid=args.kbid, show_content=args.show_content, limit=args.limit)
