"""
A/B Test: Simple vs Comprehensive Prompt
Compare both prompts with Qwen3-8B and Qwen3-4B

Test matrix:
1. Simple Prompt + Qwen3-8B
2. Comprehensive Prompt + Qwen3-8B
3. Simple Prompt + Qwen3-4B
4. Comprehensive Prompt + Qwen3-4B
"""
...
# =============================================================================
# PROMPT A: SIMPLE (3 entity types)
# =============================================================================

PROMPT_SIMPLE = """You are a PII extractor for IT Support Tickets.
...
# =============================================================================
# PROMPT B: COMPREHENSIVE (25 entity types with regex)
# =============================================================================
...
# =============================================================================
# REGEX DETECTOR (same for all tests)
# =============================================================================
...
def slm_extract(text: str, model: str, prompt: str) -> list[dict]:
    """Use SLM to extract entities with given prompt and structured output."""
    try:
        ...
        # OpenRouter returns the structured JSON in the content
        raw = response.choices[0].message.content.strip()
        data = json.loads(raw)
        
        # The schema wraps results in an "entities" key
        ...
        try:
            print(f"Warning: Structured output failed for {model}, falling back to legacy parsing. Error: {e}")
            response = client.chat.completions.create(
                model=model,
                messages=[
                    {"role": "system", "content": prompt},
...
def benchmark_combination(model: str, prompt_name: str, prompt: str, n_samples: int = 10) -> dict:
    """Benchmark a single model+prompt combination."""
    print(f"\n{'=' * 70}")
    print(f"TESTING: {model} + {prompt_name}")
    print("=" * 70)
    ...
    for i, item in enumerate(data[:n_samples]):
        ...
        # Stage 2: SLM
        t0 = time.time()
        slm_ents = slm_extract(text, model, prompt)
...
def run_ab_test(n_samples: int = 10):
    """Run A/B test: Simple vs Comprehensive prompt."""
    print("=" * 70)
    print(f"A/B TEST: Simple vs Comprehensive Prompt ({n_samples} samples)")
    print("=" * 70)
    
    results = []
    
    # Test 1: Qwen3-8B + Simple
    results.append(benchmark_combination("qwen/qwen3-8b", "Simple", PROMPT_SIMPLE, n_samples))
    
    # Test 2: Qwen3-8B + Comprehensive
    results.append(benchmark_combination("qwen/qwen3-8b", "Comprehensive", PROMPT_COMPREHENSIVE, n_samples))
    
    # Test 3: Qwen3-4B + Simple
    results.append(benchmark_combination("qwen/qwen3-4b:free", "Simple", PROMPT_SIMPLE, n_samples))
    
    # Test 4: Qwen3-4B + Comprehensive
    results.append(benchmark_combination("qwen/qwen3-4b:free", "Comprehensive", PROMPT_COMPREHENSIVE, n_samples))
    
    # Summary table
    print(f"\n{'=' * 70}")
    print("SUMMARY TABLE")
    print("=" * 70)
    print(f"{'Model':<20} {'Prompt':<15} {'F1':>8} {'Precision':>10} {'Recall':>8} {'Time':>10}")
    print("-" * 70)
    for r in results:
        print(f"{r['model']:<20} {r['prompt']:<15} {r['f1']:>7.1%} {r['precision']:>9.1%} {r['recall']:>7.1%} {r['slm_time_s']:>9.2f}s")
    print("=" * 70)
    
    # Find winner
    best = max(results, key=lambda x: x['f1'])
    print(f"\n🏆 WINNER: {best['model']} + {best['prompt']} (F1: {best['f1']:.1%})")
