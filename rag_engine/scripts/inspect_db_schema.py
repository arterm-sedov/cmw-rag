...
def get_all_metadata_paginated(store: ChromaStore, limit_per_batch: int = 1000) -> list[dict]:
    ...
    while True:
        res = store.collection.get(
            limit=limit_per_batch, offset=offset, include=["metadatas", "documents", "ids"]
        )
...
def inspect_schema(store: ChromaStore, num_samples: int = 5) -> None:
    ...
    print(
        f"Chunks per article (avg): {len(all_records) / len(unique_doc_stable_ids) if unique_doc_stable_ids else 0:.1f}"
    )
    ...
    path_like_kbids = [
        ...
        if r.get("kbId")
        and ("\\" in str(r.get("kbId")) or "/" in str(r.get("kbId")) or " " in str(r.get("kbId")))
    ]
    if path_like_kbids:
        issues.append(
            f"⚠️  {len(path_like_kbids)} records have path-like kbIds (should be numeric or normalized)"
        )
