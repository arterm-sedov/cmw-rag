...
def debug_single_file(file_path: str) -> None:
    ...
    if not target_doc:
        print(f"ERROR: Could not process file. Make sure it's a valid markdown file with kbId in frontmatter.")
        return
    ...
    try:
        ...
        if content_raw.startswith("---"):
            ...
            if len(parts) >= 3:
                ...
                print(f"  ✓ Raw frontmatter kbId: {raw_kb_id} (type: {type(raw_kb_id).__name__ if raw_kb_id else 'None'})")
                if str(raw_kb_id) != str(kb_id):
                    print(f"  ⚠ WARNING: Raw frontmatter kbId ({raw_kb_id}) != processed kbId ({kb_id})")
    ...
    if existing:
        ...
        if existing_epoch is None:
            ...
            if existing_epoch >= epoch:
                ...
                print(f"  ✓ Difference: {epoch - existing_epoch} seconds ({(epoch - existing_epoch) / 86400:.1f} days)")
        else:
        ...
        try:
            ...
            for kb_id_variant in [str(kb_id), int(kb_id) if str(kb_id).isdigit() else None]:
                if kb_id_variant is None:
                    continue
                try:
                    all_docs = store.collection.get(where={"kbId": kb_id_variant}, limit=1, include=["metadatas"])
                    if all_docs.get("metadatas"):
                        found_meta = all_docs["metadatas"][0]
                        found_doc_stable_id = found_meta.get("doc_stable_id")
                        found_kb_id = found_meta.get("kbId")
                        print(f"  ✓ Found document with exact kbId={found_kb_id} (queried as {kb_id_variant}, type: {type(kb_id_variant).__name__})")
                        ...
                        print(f"    Found metadata file_mtime_epoch: {found_meta.get('file_mtime_epoch')}")
                        found_exact = True
                        break
                except Exception as e:
                    continue
            ...
            if not found_exact and numeric_kb_id:
                ...
                try:
                    ...
                    if matching_docs:
                        print(f"  ⚠ Found {len(matching_docs)} document(s) with matching numeric kbId={numeric_kb_id}:")
                        for i, found_meta in enumerate(matching_docs[:3], 1):
                            found_kb_id = found_meta.get("kbId")
                            found_doc_stable_id = found_meta.get("doc_stable_id")
                            found_numeric = extract_numeric_kbid(found_kb_id)
                            print(f"    Match {i}: kbId={found_kb_id} (numeric={found_numeric}), doc_stable_id={found_doc_stable_id}")
                            print(f"      Expected doc_stable_id: {doc_stable_id}")
                            if found_doc_stable_id != doc_stable_id:
                                print(f"      ⚠ MISMATCH: doc_stable_id doesn't match! (file has suffix: '{found_kb_id}' vs expected '{kb_id}')")
                            print(f"      Found metadata file_mtime_epoch: {found_meta.get('file_mtime_epoch')}")
                    else:
        ...
        try:
            ...
            if sample_metas:
                ...
                for i, meta in enumerate(sample_metas[:3], 1):
                    ...
                    print(f"    Sample {i}: kbId={sample_kb_id} (type: {type(sample_kb_id).__name__}), doc_stable_id={sample_doc_stable_id}")
            else:
