...
- [ ] Re-verify D1-D10 line numbers before editing
- [ ] Confirm deep research files exist at `deep-researches/`
- [ ] Verify AGENTS.md location and read rules
...
- [ ] **Step 1:** Confirm line 6: `- продажипродажи`
- [ ] **Step 2:** Replace with two tags:
  ```yaml
    - продажи
    - продукт
  ```
- [ ] **Step 3:** Remove duplicate `- продукт` if present
- [ ] **Step 4:** Verify YAML block opens with `---` (line 1) and closes with `---`
...
- [ ] **Step 1:** Confirm no `---` YAML block (lines 1-4)
- [ ] **Step 2:** Prepend:
  ```yaml
  ---
  ...
  ---
  ```
- [ ] **Step 3:** Delete redundant bold-text metadata lines
- [ ] **Step 4:** Add H1 anchor: `{: #app_c_pack_overview }`
- [ ] **Step 5:** Add H2 anchor: `{: #app_c_overview }`
...
- [ ] **Step 1:** Read lines 16-23, confirm plain text
...
- [ ] **Step 1:** Fix line 63: `[«Приложение C: доказательство готовности — референс-стек Comindware»](./20260325-research-appendix-c-cmw-existing-work-ru.md#app_c_pack_overview)`
- [ ] **Step 2:** Fix line 74: same link in topic table
- [ ] **Step 3:** Fix line 93: same link in Q→doc navigation
- [ ] **Step 4:** Fix line 128: same link in source mapping table
...
- [ ] **Main:M** (`research-report-methodology-main-ru.md`): Remove duplicate `методология`- [ ] **Main:S** (`research-report-sizing-economics-main-ru.md`): Remove duplicate `сайзинг`
- [ ] **Appendix A** (`research-appendix-a-index-ru.md`): Remove duplicate `методология`
- [ ] **Appendix D** (`research-appendix-d-security-observability-ru.md`): Remove duplicates `безопасность`, `комплаенс`, `наблюдаемость`
...
- [ ] **Step 1:** Find §1б (Ведомость файлов)
- [ ] **Step 2:** Add:
  ```markdown
...
- [ ] Add to each file's `## Связанные документы` section:
  ```markdown
...
- [ ] **Step 1:** Find all anchors with `sizing_` prefix (grep `{: #sizing_`)
- [ ] **Step 2:** Find all anchors with `method_` prefix (grep `{: #method_`)
- [ ] **Step 3:** Replace each `#sizing_<name>` with `#app_e_<name>`
- [ ] **Step 4:** Replace each `#method_<name>` with `#app_e_<name>`
- [ ] **Step 5:** Update H1 anchor from `#app_e_root` (already correct)
- [ ] **Step 6:** Count total changes (~44 anchors expected)
...
- [ ] **Step 1:** Find all links pointing to `#sizing_*` in Appendix E context
- [ ] **Step 2:** Find all links pointing to `#method_*` in Appendix E context
- [ ] **Step 3:** Update each link to use new `#app_e_*` prefix
- [ ] **Step 4:** Update D7 stub link from `#sizing_russian_market` to `#app_e_russian_market`
...
Verify:
- [ ] YAML blocks valid in Commercial Offer, Appendix C
- [ ] No duplicate tags
- [ ] Appendix A has 4 working Appendix C links
- [ ] Appendix C has 5 working sibling links
- [ ] Appendix E anchors renamed to `app_e_*` (~44 anchors)
- [ ] Incoming links updated (~8 links)
...
- [ ] **Step 1:** Find insertion point (before `## Тарифы российских облачных провайдеров ИИ`)
- [ ] **Step 2:** Insert stub section:
  ```markdown
...
- [ ] **Step 3:** Verify anchor `#sizing_russian_market` now exists in sizing report
...
- [ ] Replace bold-text reference with: `зафиксированы в _«[Основной отчёт: сайзинг и экономика — Российский рынок](./20260325-research-report-sizing-economics-main-ru.md#sizing_russian_market)»_`

---

### Task 2.3 — Verify Major Anchors

- [ ] `#app_b_pack_overview` exists in Appendix B
- [ ] `#app_d__pack_overview` exists in Appendix D
- [ ] `#method_pack_overview` exists in Main:M
- [ ] `#app_a_fx_policy` exists in Appendix A
...
Verify:
- [ ] All 4 broken `#sizing_russian_market` references now resolve
- [ ] Appendix C correctly links to sizing report
- [ ] Major anchors verified
...
- [ ] Convert plain `[brackets]` to `_«[Title](url)»_` format in body text
...
- [ ] Add at start: `**Валюта:** Курс USD/RUB для иллюстративных ориентиров — см. [приложение A](./20260325-research-appendix-a-index-ru.md#app_a_fx_policy).`
...
- [ ] **Step 1:** Find the USD figure in context
- [ ] **Step 2:** Add RUB equivalent or FX reference:
  ```markdown
...
Verify:
- [ ] `## Источники` present in Commercial Offer
- [ ] Citations use `_«[...]»_` format
- [ ] FX policy reference present
- [ ] USD figure in Main:Sizing has RUB conversion
...
- [ ] Insert after `## Зачем это бизнесу (P&L и контроль)`:
  ```markdown
...
- [ ] Insert before `## Где искать полноту`:
  ```markdown
...
- [ ] Insert after "Типовые компромиссы":
  ```markdown
...
- [ ] Audit all "43%" references in:
  - Main:Sizing
  - Exec:Sizing
  - Appendix D
  - Appendix E
- [ ] Add disclaimer where appropriate: `_Примечание: источник опроса CMO Club не удалось верифицировать напрямую; цифра приводится по вторичным материалам._`
...
- [ ] Verify usage in Main:Sizing matches deep research context
...
- [ ] Appendix E anchors normalized to `app_e_*` (~44 anchors)
- [ ] Incoming links to Appendix E updated (~8 links)
- [ ] `## Источники` in Commercial Offer
- [ ] FX policy referenced in Commercial Offer
- [ ] USD figures have RUB conversion (D11)
- [ ] Role routing in both exec summaries **focuses on SELLS/APPROVES**
- [ ] Market context emphasizes **demand for implementation expertise**
- [ ] Objection handling addresses **buying implementation capability**
- [ ] No "субпроцессоры" terminology — use "обработчики данных"
- [ ] "43%" figures flagged or verified
